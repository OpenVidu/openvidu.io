...
- **A copyleft license to account for.** Janus is GPL v3, worth checking against your own
  project's licensing model before you build on it (Meetecho also sells a commercial license if GPL
  doesn't fit).
...
| Server/REST API | Full [server API](docs/reference/room-service-api.md) | Admin/Monitor API only (session-level, not app-level) |
...
gateway integrations (although that's also possible through
[LiveKit's SIP API :fontawesome-solid-external-link:{.external-link-icon}](https://docs.livekit.io/reference/telephony/sip-api/){:target="_blank"}),
one-to-many streaming, and other topologies its plugins already cover. Teams building one of those
use cases, or something custom enough that a room-centric platform like OpenVidu doesn't fit, get
real value from working directly with Janus's core-plus-plugins design.
