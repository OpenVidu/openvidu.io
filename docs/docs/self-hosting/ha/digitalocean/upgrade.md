...
8. After upgrading all your Master Nodes, **you need to delete the Media Nodes** to apply the changes to run the Media Nodes with the new version. Go to your DigitalOcean web, Droplets tab, select the Media Nodes instances and terminate them. The lambda will make new media nodes, or the autoscale pool if you have fixed media nodes.
