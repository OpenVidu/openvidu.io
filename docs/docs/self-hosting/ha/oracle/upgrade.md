...
5. **Repeat steps 1 to 4 on each of the remaining Master Nodes of your deployment.** This is important because all 4 Master Nodes need to be running the same version of OpenVidu.

6. After the final intermediate upgrade completes on all 4 Master Nodes, **you need to terminate the Media Nodes** to apply the changes. Go to the OCI Console, navigate to the **Compute → Instance Pools** section, open `<STACK_NAME>-media-pool`, and terminate the existing Media Node instances. The Instance Pool will automatically launch new Media Nodes with the updated configuration.

7. Once the Media Nodes are up and running, restart OpenVidu High Availability by running the following command **on every Master Node**:
