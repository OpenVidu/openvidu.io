...
> All client side operations are exemplified using the [LiveKit JS Client SDK](https://docs.livekit.io/reference/client-sdk-js/modules.html){target="\_blank"}. For other client SDKs, refer to the corresponding LiveKit reference documentation.
...
> [:octicons-arrow-right-24: Reference docs](https://docs.livekit.io/frontends/build/authentication/){target="\_blank"}
...
> [:octicons-arrow-right-24: Reference docs](https://docs.livekit.io/intro/basics/connect/){target="\_blank"}
...
> [:octicons-arrow-right-24: Reference docs](https://docs.livekit.io/intro/basics/connect/#disconnect-from-a-room){target="\_blank"}
...
> [:octicons-arrow-right-24: Reference docs](https://docs.livekit.io/transport/media/publish/){target="\_blank"}
...
> [:octicons-arrow-right-24: Reference docs](https://docs.livekit.io/transport/media/publish/#Mute-and-unmute){target="\_blank"}
...
> [:octicons-arrow-right-24: Reference docs](https://docs.livekit.io/reference/client-sdk-js/classes/LocalParticipant.html#unpublishTrack){target="\_blank"}
...
First of all, you can set Room metadata that will be available for all clients in the `Room` object. You do so in your application server when calling the [`CreateRoom` API](https://docs.livekit.io/reference/other/roomservice-api/#CreateRoom){target="\_blank"} (available for all LiveKit Server SDKs and the HTTP Server API). The Room metadata will be available in the client side like this:

```typescript
console.log(room.metadata);
```

You can update the Room metadata at any time from your application server with the [`UpdateRoomMetadata` API](https://docs.livekit.io/reference/other/roomservice-api/#UpdateRoomMetadata){target="\_blank"} (available for all LiveKit Server SDKs and the HTTP Server API). The client side will be notified of the change with the `roomMetadataChanged` event of the `Room` object:
...
Secondly, you can also set Participant metadata. You do so when creating an [access token](https://docs.livekit.io/frontends/build/authentication/){target="\_blank"} in your application server, setting `metadata` field of the JWT.

Participants can also update their own metadata from the client side, if their access token was created with [grant `canUpdateOwnMetadata`](https://docs.livekit.io/frontends/reference/tokens-grants/#video-grant){target="\_blank"}.
...
Finally, you can send messages to Participants in the Room using the [`LocalParticipant.publishData`](https://docs.livekit.io/reference/client-sdk-js/classes/LocalParticipant.html#publishData){target="\_blank"} method:
...
The [`DataPublishOptions`](https://docs.livekit.io/reference/client-sdk-js/types/DataPublishOptions.html){target="\_blank"} allow setting the reliability of the message (depending on the nature of the message it can be sent as a reliable or lossy message), a topic to easily filter messages, and the participants that will receive the message.
...
> [:octicons-arrow-right-24: Reference docs](https://docs.livekit.io/transport/data/){target="\_blank"}
...
- [**RoomService**](https://docs.livekit.io/reference/other/roomservice-api/#RoomService-APIs){target="\_blank"}: to manage Rooms and Participants.
- [**Egress**](https://docs.livekit.io/reference/other/egress/api/){target="\_blank"}: to manage egress operations.
- [**Ingress**](https://docs.livekit.io/reference/other/ingress/api/){target="\_blank"}: to manage ingress operations.
...
You can also create custom screen tracks, for example capturing the audio of the screen and fine-tuning the video capture options (checkout the [ScreenTrackOptions](https://docs.livekit.io/reference/client-sdk-js/interfaces/ScreenShareCaptureOptions.html){target="\_blank"} interface for detailed information):
...
> [:octicons-arrow-right-24: Reference docs](https://docs.livekit.io/transport/media/screenshare/){target="\_blank"}
...
> [:octicons-arrow-right-24: Reference docs](https://docs.livekit.io/transport/media/ingress-egress/egress/){target="\_blank"}
...
> [:octicons-arrow-right-24: Reference docs](https://docs.livekit.io/transport/media/ingress-egress/ingress/){target="\_blank"}

### IP Cameras

With OpenVidu you can ingest **RTSP** streams into your Rooms. To do so, simply use the [Ingress API](https://docs.livekit.io/transport/media/ingress-egress/ingress/){target="\_blank"} to create and ingress of input type `URL`, providing the IP camera RTSP URL as value:
...
=== ":material-api:{.icon .lg-icon .tab-icon} Server API"
    ...
    1. Consume the Ingress API directly: [:octicons-arrow-right-24: Reference Docs](https://docs.livekit.io/reference/other/ingress/api/){target="\_blank"}
...
> [:octicons-arrow-right-24: Reference docs](https://docs.livekit.io/intro/basics/rooms-participants-tracks/webhooks-events/){target="\_blank"}
