...
Secondly, you can also set Participant metadata. You do so when creating an [access token](../reference/access-tokens.md) in your application server, setting `metadata` field of the JWT.

Participants can also update their own metadata from the client side, if their access token was created with [grant `canUpdateOwnMetadata`](../reference/access-tokens.md#video-grants).
