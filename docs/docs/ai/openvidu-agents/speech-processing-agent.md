...
# How the agent runs its Rooms when the provider is local (vosk, sherpa) [thread, process]
# - thread: all Rooms run in the same process sharing the same model. Lower memory usage, but upper limit of 20 concurrent audio tracks per process.
# - process: each Room runs in its own process with its own copy of the model. Higher memory usage, but upper limit of concurrent audio tracks scales with CPUs.
job_executor: thread
