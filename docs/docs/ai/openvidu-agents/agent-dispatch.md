...
=== ":simple-nodedotjs:{.icon .lg-icon .tab-icon} Node.js"
    ...
    Using [LiveKit Node SDK](https://docs.livekit.io/reference/server-sdk-js/){target="\_blank"}
...
=== ":material-api:{.icon .lg-icon .tab-icon} Server API"
    ...
    1. Consume the Agent Dispatch API directly:
        ...
        > You need as `VALID_AUTHORIZATION_TOKEN` a token with `room` and `roomAdmin` permissions. Visit LiveKit docs: [Creating a token](https://docs.livekit.io/frontends/build/authentication/#alternative-manual-token-generation){target="\_blank"}

        <br>

    2. Use the [livekit-cli](https://docs.livekit.io/intro/basics/cli/){target="\_blank"}:
...
To create a Participant's token with Agent dispatch, you just need to include in the token the proper `RoomConfiguration` options, specifically the **`agents`** property. Visit [LiveKit docs](https://docs.livekit.io/frontends/reference/tokens-grants/#room-configuration){target="\_blank"} to learn how.