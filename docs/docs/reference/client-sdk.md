...
description: "How an OpenVidu application client connects to a Room, publishes and subscribes to Tracks, exchanges data, and reacts to every Room event."
---
...
It represents one participant inside a Room. It connects with an [access token](./access-tokens.md), publishes camera, microphone or screen, subscribes to what others publish, and raises events as the Room changes around it.
...
!!! info
    ...
    - [React Native :fontawesome-solid-external-link:{.external-link-icon}](https://htmlpreview.github.io/?https://raw.githubusercontent.com/livekit/client-sdk-react-native/main/docs/modules.html){:target="_blank"}
    - [Unity :fontawesome-solid-external-link:{.external-link-icon}](https://livekit.github.io/client-sdk-unity/api/LiveKit.html){:target="_blank"}
    - [Unity (WebGL) :fontawesome-solid-external-link:{.external-link-icon}](https://livekit.github.io/client-sdk-unity-web/api/LiveKit.html){:target="_blank"}
    - [Node.js :fontawesome-solid-external-link:{.external-link-icon}](https://docs.livekit.io/reference/client-sdk-node/){:target="_blank"}
    - [Rust :fontawesome-solid-external-link:{.external-link-icon}](https://docs.rs/livekit/latest/livekit/){:target="_blank"}
    - [C++ :fontawesome-solid-external-link:{.external-link-icon}](https://docs.livekit.io/reference/client-sdk-cpp/){:target="_blank"}
    - [Python :fontawesome-solid-external-link:{.external-link-icon}](https://docs.livekit.io/reference/python/livekit/rtc/index.html){:target="_blank"}
    - [Go :fontawesome-solid-external-link:{.external-link-icon}](https://github.com/livekit/server-sdk-go){:target="_blank"}
    - [ESP32 :fontawesome-solid-external-link:{.external-link-icon}](https://livekit.github.io/client-sdk-esp32/){:target="_blank"}
...
2. Register Room event handlers. Usually the best place to do this is before connecting to avoid missing events.
3. Connect to the Room. `OPENVIDU_URL` is your deployment's WebSocket endpoint: `ws://localhost:7880` for a [local deployment](../self-hosting/local.md), `wss://your-domain` in production. The token comes from your application server, see [access tokens](./access-tokens.md).
4. Publish the camera and the microphone of the device, so the other participants can subscribe to them.
...
The `Disconnected` event (for the local participant) and `ParticipantDisconnected` event (for remote participants) carry a [`DisconnectReason` :fontawesome-solid-external-link:{.external-link-icon}](https://docs.livekit.io/reference/client-sdk-js/enums/DisconnectReason.html){:target="_blank"}. You can handle them separately in your UI if you need:
...
- **Resume**: the client re-establishes the signalling connection and performs an [ICE restart :fontawesome-solid-external-link:{.external-link-icon}](https://developer.mozilla.org/en-US/docs/Web/API/WebRTC_API/Session_lifetime#ice_restart){:target="_blank"} on the existing session. The participant stays in the Room, tracks are not republished and no `Reconnecting` event is emitted. Users may notice a pause of a few seconds in the video. This is the usual case.
...
2. `LocalTrackUnpublished` for each of your tracks that was unpublished.
3. `Reconnecting`.
4. The client rejoins the Room.
5. `Reconnected`.
6. `ParticipantConnected` for everyone currently in the Room.
7. `LocalTrackPublished` as your tracks are republished.
...
- A `reconnecting` state does not mean the Room is over for the participant. In an [Elastic or High Availability deployment](../self-hosting/production-ready/fault-tolerance.md), a Media Node failure is recovered by rebuilding the Room on a healthy node in a few seconds.
- Reconnecting does not require a new token. The client uses the token that OpenVidu refreshed while it was connected, so the application server is not involved. See [token lifecycle](./access-tokens.md#token-lifecycle).
...
- **The token decides what may be published.** The `canPublishSources` grant can allow the camera and the microphone but not the screen, and revoking `canPublish` while connected unpublishes everything that Participant had published. See [video grants](./access-tokens.md#video-grants).
...
You can also create custom screen tracks, for example capturing the audio of the screen and fine-tuning the video capture options (checkout the [ScreenShareCaptureOptions :fontawesome-solid-external-link:{.external-link-icon}](https://docs.livekit.io/reference/client-sdk-js/interfaces/ScreenShareCaptureOptions.html){:target="_blank"} interface for detailed information):
...
!!! warning "SDK support"
    Virtual backgrounds are only supported in the JavaScript SDK (blur and background image) and the Swift SDK (only blur: [BackgroundBlurVideoProcessor :fontawesome-solid-external-link:{.external-link-icon}](https://docs.livekit.io/reference/client-sdk-swift/documentation/livekit/backgroundblurvideoprocessor/){:target="_blank"}).
...
!!! tip "Toggling the effect without visual artifacts"
...
!!! warning "Adaptive stream in JavaScript"
...
2. Publisher side. Stops sending the layers that nobody is consuming. Defaults to `false`.
3. Publisher side. Publishes several versions of each video Track. Defaults to `true`: set it to `false` to publish a single encoding.
4. Using `vp9` or `av1` activates SVC, with three spatial and temporal layers. Defaults to `true` when those codecs are used.
...
- **Room metadata** is a single string for the whole Room. Only your application server can set it, with [`CreateRoom`](./room-service-api.md#rooms) or [`UpdateRoomMetadata`](./room-service-api.md#rooms). Clients read `room.metadata` and listen for event `RoomMetadataChanged`.
- **Participant attributes** are a key-value store, so a single key can be updated without resending the rest. **Participant metadata** is the single-string equivalent. Both can be given an initial value in the [access token](./access-tokens.md), so they are available the moment the participant connects, and a participant can change its own if its token carries grant [`canUpdateOwnMetadata`](./access-tokens.md#video-grants).
...
Reliable packets are retransmitted and delivered in order; lossy packets are sent once. Each packet holds up to 15 KiB, and your application server can publish them too with [`SendData`](./room-service-api.md#data).

## Room events

Handlers are registered on the Room with `room.on(...)`. These are the events of the [`RoomEvent` :fontawesome-solid-external-link:{.external-link-icon}](https://docs.livekit.io/reference/client-sdk-js/enums/RoomEvent.html){:target="_blank"} enum, grouped by what they tell you.

Participants emit their own [`ParticipantEvent` :fontawesome-solid-external-link:{.external-link-icon}](https://docs.livekit.io/reference/client-sdk-js/enums/ParticipantEvent.html){:target="_blank"} events for the subset that concerns them, so you can listen per participant instead of filtering Room-wide events. The last column of each table says where an event is emitted, including the few that exist only on the Participant.
...
| `ConnectionStateChanged` | Any transition between the [ConnectionStates :fontawesome-solid-external-link:{.external-link-icon}](https://docs.livekit.io/reference/client-sdk-js/enums/ConnectionState.html){:target="_blank"} | Room |
...
| `TranscriptionReceived` | A transcription segment arrived. This is how [live captions](../ai/live-captions.md) reach the client | Room, Participant |
...
- [Application client tutorials](../tutorials/application-client/index.md): complete application clients in different frameworks.
- [Access tokens reference](./access-tokens.md): what a participant is allowed to do inside a Room.
- [Room Service API reference](./room-service-api.md): the same Room, managed from your application server.
