...
At OpenVidu we've applied hardening recommendations while always keeping platform usability in mind. From [Enable Security's recommendations :fontawesome-solid-external-link:{.external-link-icon}](https://www.enablesecurity.com/blog/coturn-security-configuration-guide/){:target="_blank"}, we've applied the following:
...
2. **To prevent the TCP-to-peer abuse vector**, peer traffic is restricted to UDP-only relay: TCP allocations ([RFC 6062 :fontawesome-solid-external-link:{.external-link-icon}](https://datatracker.ietf.org/doc/html/rfc6062){:target="_blank"}) are disabled by default, though they can be enabled via configuration. By preventing the TURN server from establishing outgoing TCP connections to peers, the door is closed to an attacker using the relay as a TCP proxy toward internal services. This doesn't entirely prevent an attacker with valid credentials from abusing the infrastructure, but it drastically reduces the attack vector.
