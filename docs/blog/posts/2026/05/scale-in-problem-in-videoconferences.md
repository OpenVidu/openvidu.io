...
| :material-google-cloud: [**GCP** :fontawesome-solid-external-link:{.external-link-icon}](https://docs.cloud.google.com/compute/docs/instance-groups){:target="_blank"} | Scale-out-only MIG + Cloud Run Function + per-node cron job | Full control over which instance is selected and when. No dependency on provider-level termination hooks. | Polling-based coordination (1-min cron). Slightly higher complexity in the coordination layer. |
