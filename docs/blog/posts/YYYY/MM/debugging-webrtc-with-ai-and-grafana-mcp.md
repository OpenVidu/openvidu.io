...
description: "We give a Claude Code agent nothing but read-only Grafana and a broken OpenVidu cluster, then watch it find each root cause in a reproducible case study."
...
![Debugging WebRTC with an AI agent and Grafana MCP: read-only Grafana, a broken cluster, and an agent that walks the metrics to find each root cause](/assets/images/blog/YYYY/MM/debugging-webrtc-with-ai-and-grafana-mcp/poster.webp){ width=100% }

What if you gave an AI agent nothing but **read-only access to your Grafana**, pointed it at a WebRTC cluster it had never seen, and asked what was broken? No shell, no source code, no config files, nothing but the dashboards and logs any on-call engineer would stare at. Could it actually find the root cause?
...
The hardest part of debugging WebRTC problems in production is understanding the network environment and the state of the various services: CPU load, memory, bandwidth, logs.

On top of that, most web developers' expertise is focused on microservices and simpler protocols like HTTPS, frontend, or database queries, so they rarely worry about the network environment or where their software is going to run (even though they should). And it makes perfect sense: the business logic is usually the higher priority, since the real production problems under real demand don't tend to show up until the system is already live.

If debugging problems in a plain web application is already painful, adding WebRTC to the equation makes it even worse.

Building tools that are ready to debug WebRTC and this kind of thing is very hard if you have no prior experience with the technology. The lack of observability, the scalability problems, and the complexity of all the protocols put a lot of people off. At OpenVidu we care a lot about that new developer who has never handled a media stream in their life, and we know how costly it is not just to operate a cluster, but to understand what's going on when something is failing.

In that effort to improve the WebRTC experience, at OpenVidu we wanted to run an experiment. Internally we have tools to simulate clusters and see what happens when the network goes bad or the CPU gets congested... and we asked ourselves: what would happen if we connected an LLM agent to our observability stack? Can it really tell what's going on?

## What if we hand an AI agent a broken cluster?

First, a one-line refresher: an agent harness gives an LLM autonomous capabilities, so it can run commands, write files, and act on a computer on its own.

We decided to connect an agent harness (in this blog's case, Claude Code) to a local OpenVidu Single Node deployment and **broke it on purpose**. Then we handed a fresh Claude Code session a single sentence: *"our video calls are broken since about 14:30, people join but nobody can see or hear anyone."*
...
Well, in this post you'll find multiple simulated failure scenarios on a local deployment, the prompt used in each case and the answer, so you can see how much this kind of tooling can ease the job of debugging deployments, and how OpenVidu wires it all together so you can use it with ease.

## The test rig

To keep this experiment easy to replicate, and nimble enough to keep adding new scenarios, we use a project of our own: [openvidu-fake-vm](https://github.com/OpenVidu/openvidu-fake-vm){:target="_blank"}. It lets us launch a Docker container that pretends to be a virtual machine, where we can deploy OpenVidu Single Node with no trouble at all.

On that machine we inject faults to simulate different scenarios. As we drive the cluster through its API, errors are produced, and the whole observability stack records the logs of every deployed service.

By pointing the Grafana MCP (Model Context Protocol: the standard way to give an agent access to a tool, here Grafana) at that stack, we can then diagnose the problem and test whether Claude can find the cause or not.

The agent's session gets the operator's one-sentence complaint and the Grafana URL, and nothing else, with no hint that a fault even exists.

We ran it with `--strict-mcp-config` (so no other MCP could sneak in) and fully disabled the Bash and file tools.

Each fault we ran twice: **bare MCP** (control) and **bare MCP + the `openvidu-grafana-triage` skill** (treatment). Same model, same prompt, same cluster. The only difference is whether the session carried one page of OpenVidu domain knowledge.
...
- **This is a lab.** A single Dockerized node with synthetic load is not your production network; ICE behind real NAT behaves differently, and our sample is one run per arm. Treat it as illustrative, not a benchmark.
...
- The VM ([`openvidu-fake-vm`](https://github.com/OpenVidu/openvidu-fake-vm){:target="_blank"}) comes up on the fixed IP `10.5.0.3`, and the public wildcard `playground.openvidu-local.dev` resolves *exactly there*.
...
Now pick a fault. `./scenario.sh list` shows the menu; then one command leaves the cluster broken the right way, it warms up a healthy baseline first and *then* injects the fault, so the metrics tell the honest story of "it worked, then it didn't," and prints the operator's complaint, ready to paste:
...
And one last thing: this is only a preview. At OpenVidu we're working on a much more complete MCP, built from the ground up to diagnose deployments, and what you've seen here is just a taste of where we're headed. It's coming soon, so if this caught your interest, follow OpenVidu's releases and blog so you don't miss it.
