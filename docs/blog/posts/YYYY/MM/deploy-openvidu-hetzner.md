...
description: "Deploy a self-hosted OpenVidu video conferencing server on a Hetzner Cloud VM with one command, automatic HTTPS and a test bill measured in cents."
...
![OpenVidu servers inside a Hetzner cloud serving a video call](/assets/images/blog/YYYY/MM/deploy-openvidu-hetzner/poster.png){ width=100% }
...
The server and its firewall are created in the <a href="https://console.hetzner.com/" target="_blank">Hetzner Cloud console</a>, and everything from there on happens over SSH inside the instance. The result is a production-grade video conferencing stack with valid HTTPS, running <a href="/meet/">OpenVidu Meet</a>, reachable from any browser. Hetzner bills by the hour, so if the goal is just to try OpenVidu, the server can be deleted at the end and the whole experiment costs cents.
...
![Hetzner usage bill for this post's deployment, seven cents total](/assets/images/blog/YYYY/MM/deploy-openvidu-hetzner/final-bill.png){ width=80% }
...
![Creating a new project in the Hetzner Cloud console](/assets/images/blog/YYYY/MM/deploy-openvidu-hetzner/create-project.png){ width=80% }
...
2. **Location**: whichever is closest to you. The German and Finnish datacenters are the cheapest; the US and Singapore cost slightly more.
...
5. **SSH keys**: click **Add SSH key**. In the **SSH key** field, paste the exact output of the `cat` command above — one line starting with `ssh-ed25519` and ending with `user@host`. Never paste the private key (the file without `.pub`). Fill **Name** with anything that identifies the key and confirm with **Add SSH key**:
    ...
    ![The Add an SSH key dialog in the Hetzner console](/assets/images/blog/YYYY/MM/deploy-openvidu-hetzner/ssh-key-modal.png){ width=60% }
...
![Creating a firewall in the Hetzner Cloud console](/assets/images/blog/YYYY/MM/deploy-openvidu-hetzner/create-firewall.png){ width=80% }
...
![The six inbound rules of the OpenVidu firewall in the Hetzner console](/assets/images/blog/YYYY/MM/deploy-openvidu-hetzner/firewall-rules.png){ width=100% }

Before clicking **Create Firewall**, use the **Apply to** section of the same form to attach it to the server in one step.

The [full port table](/docs/self-hosting/single-node/on-premises/install.md#port-rules) in the docs lists three more **optional** ports. This guide skips them; each is one more rule away if the deployment ever needs it:
...
![The server's public IPv4 address in the Hetzner server list](/assets/images/blog/YYYY/MM/deploy-openvidu-hetzner/server-ip.png)
...
![The OpenVidu Meet console running on the same bare IP](/assets/images/blog/YYYY/MM/deploy-openvidu-hetzner/meet-console.png){ width=100% }
...
- **The OpenVidu Platform API**, based on LiveKit. The `LIVEKIT_URL`, `LIVEKIT_API_KEY` and `LIVEKIT_API_SECRET` values grepped above are what an application uses to create rooms and tokens programmatically, with [any LiveKit SDK](/docs/developing-your-openvidu-app/index.md).
- **The OpenVidu Meet API**, a higher-level [REST API](/meet/embedded/reference/rest-api.md) to manage rooms, recordings and users without touching the LiveKit layer. Generate its API key from the console's **Embedded** section and authenticate requests with the `X-API-KEY` header.
...
- **Point a domain at it.** Create an `A` record for, say, `video.yourdomain.com` pointing to the server IP, then set `DOMAIN_NAME` in `/opt/openvidu/config/openvidu.env` and restart the service. [Changing the configuration](/docs/self-hosting/configuration/changing-config.md) covers the details. Certificates for a proper FQDN are the recommended setup for production.
...
Why bother? Because this same server can do much more than a demo call: recordings stored in its built-in MinIO, [AI services](/docs/ai/overview.md) like live captions and transcription, RTMP stream ingestion, and a complete video calling product — [OpenVidu Meet](/meet/index.md) — ready to embed into your own application.
...
This post took the shortest honest path, which means it skipped options you might need. The [on-premises installation guide](/docs/self-hosting/single-node/on-premises/install.md) covers all of them: custom and ZeroSSL certificates, running behind your own proxy, plain Docker Compose installation, and a [non-interactive mode](/docs/self-hosting/single-node/on-premises/install.md#non-interactive-installation) to bake the whole install into cloud-init or Terraform.

And a single node is only the first of the [deployment types](/docs/self-hosting/deployment-types.md). When one machine stops being enough, [OpenVidu Elastic](/docs/self-hosting/elastic/index.md) scales media nodes with demand and [OpenVidu High Availability](/docs/self-hosting/ha/index.md) removes the single points of failure. The single node deployed here uses the same configuration model, so nothing learned today gets thrown away.

If you would like to explore this further, two good next steps could be:

- Compare the [deployment options](/docs/self-hosting/deployment-types.md) that fit a real workload, from this single node to elastic and HA setups.
- If what you actually need is video calls inside your own product, look at [embedding OpenVidu Meet](/meet/embedded/intro.md) with your deployment's API key, or build directly on [OpenVidu Platform](/docs/developing-your-openvidu-app/index.md) with the LiveKit SDKs. You already have everything both require.
