...
Novakid pairs children with real teachers for live one-on-one and small-group English lessons, wrapped in a gamified classroom — interactive exercises, games and activities running alongside the video the whole time. Andrei is blunt about what that means for the infrastructure underneath it:
...
That's a familiar shape: the media server itself is only the beginning. Everything around it — balancing, scaling, recording, monitoring — is its own engineering project, and Novakid was maintaining all of it on top of an open-source component instead of teaching English.
...
For a company already running Kurento, the move to OpenVidu wasn't a leap into the unknown:
...
Asked what features of the architecture matter most day to day, Andrei's answer covers deployment, scaling and — refreshingly honestly — a real limitation they found:
...
A blue-green rollout at the application level, not a maintenance window — that's the difference between a migration your users notice and one they don't.
...
- **"Vendor-managed but self-hosted" is a real middle ground.** Not raw WebRTC, not metered SaaS — a supported platform they still fully control and run on their own AWS account.
- **Scheduled autoscaling pays for itself.** Knowing lesson demand in advance lets Novakid's infrastructure "breathe" — scaling up before peak hours and back down after, instead of holding idle capacity around the clock.
