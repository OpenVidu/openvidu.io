...
- [Simulcast](https://docs.livekit.io/transport/media/advanced/#video-simulcast){target="_blank"} for VP8 and H264 video codecs.
- [Scalable Video Coding (SVC)](https://docs.livekit.io/transport/media/advanced/#video-codec-support){target="_blank"} for VP9 and AV1 video codecs.
- [Dynamic Broadcasting (Dynacast)](https://docs.livekit.io/transport/media/advanced/#dynacast){target="_blank"} for minimizing bandwidth consumption. It pauses the publication of any video layer that is not being consumed by any subscriber.
- [Adaptive Stream](https://docs.livekit.io/transport/media/subscribe/#Adaptive-stream){target="_blank"} for UI-based video quality optimization. It sends to each user the minimum bits needed to display high-quality rendering based on the size of the video player. If a video player is hidden, the video stream is paused. This allows you to scale your application to large video rooms with thousands of users.
- [Audio RED (REDundant Encoding)](https://docs.livekit.io/transport/media/advanced/#audio-red){target="_blank"} and [Hi-fi audio](https://docs.livekit.io/transport/media/advanced/#hi-fi-audio){target="_blank"} for crisp, clear, high-quality audio streams.
