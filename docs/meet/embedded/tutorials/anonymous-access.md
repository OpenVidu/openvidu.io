...
This tutorial uses the same application as the [WebComponent tutorial](./webcomponent.md), so the room creation/deletion and the WebComponent embedding are explained there. Here we focus on the **anonymous access links**: where they come from and how the application uses them to access a room.
