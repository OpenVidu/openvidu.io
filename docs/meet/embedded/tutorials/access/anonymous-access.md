...
It uses the same application as the [WebComponent tutorial](../integration/webcomponent.md) (built with **Node.js and Express** for the backend and plain **HTML/CSS/JavaScript** for the frontend), but here we focus on **how the anonymous access links are obtained and used** to let participants into a room.
...
This tutorial uses the same application as the [WebComponent tutorial](../integration/webcomponent.md), so the room creation/deletion and the WebComponent embedding are explained there. Here we focus on the **anonymous access links**: where they come from and how the application uses them to access a room.

---

### Where the anonymous access links come from

Anonymous access links are generated by OpenVidu Meet when a room is created. The room object returned by the API (`POST /rooms`, see the [Direct Link tutorial backend section](../integration/direct-link.md#backend)) includes an `access` object with the anonymous links for each role:
...
The room list template exposes the two anonymous access links as **Access as Moderator** and **Access as Speaker** buttons:
