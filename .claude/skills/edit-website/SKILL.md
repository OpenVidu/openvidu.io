...
The source tree has no version in paths: `mike` builds the site into a version folder on `gh-pages`, then `ovweb` rewrites links/canonicals/`llms.txt`/`sitemap.xml`, moves the non-versioned pages to root, and writes the generated redirect pages (step table in `publish-tool/README.md`, "What a publish does").
