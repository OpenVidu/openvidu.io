...
- **Registered in `llmstxt`** — every post (draft or published) has its own line in the `llmstxt` plugin `sections` in `mkdocs.yml` (`- blog/posts/YYYY/MM/<slug>.md: <description>`, with `YYYY/MM` literal on drafts — it resolves, since the draft file really lives at that path). Posts are listed individually; a missing entry means no `llms.txt` entry → Medium severity. An entry whose path doesn't match the post's actual location → High severity (silently skipped at build time).
