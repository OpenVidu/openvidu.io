...
- **File, naming & assets** — a **published** post lives at `docs/blog/posts/<year>/<month>/<slug>.md`, where `<year>/<month>` equals the frontmatter `date` **and** the filename equals the frontmatter `slug`; its asset folder is `docs/assets/images/blog/<year>/<month>/<slug>/` (same year/month). A **draft** is identical except that the year/month path segments are the **literal placeholder `YYYY/MM`** (real directories with those names): post at `docs/blog/posts/YYYY/MM/<slug>.md`, assets at `docs/assets/images/blog/YYYY/MM/<slug>/`, and a **temporary real date** (the draft's creation day) in the frontmatter — so draft branches build with zero warnings. Do NOT flag the placeholder folders or the temporary date on a draft (drafts live on working branches; the date is replaced at publish). DO flag: a published post whose date doesn't match its folder (post or assets), a draft mixing placeholder and real year/month paths, a date-prefixed filename (old convention), or a literal `date: YYYY-MM-DD` string (aborts the build). Any mismatch → High severity.
- **Frontmatter present and complete** — `title`, `draft`, `date`, `slug`, `description`, `categories`, `tags`, `authors`, and the `hide` block. `title` and `description` are **required** — the build fails on any post missing either (the llmstxt hook), and `description` also feeds search snippets, `og:description` and JSON-LD → absence of either is High severity. `cover_image` is **recommended**: a raster image (png/jpg/webp, **not** svg) that actually exists inside this post's asset folder — flag a missing or broken value.
- **Covered by `llmstxt`** — the `Blog:` section in `mkdocs.yml` is the **glob** `blog/posts/*/*/*.md`, which matches published (`<year>/<month>`) and draft (`YYYY/MM`) paths alike, so a post at the conventional path needs **no `mkdocs.yml` change — do not ask for one**. A post outside that layout (an extra folder level, a file directly under `posts/`) falls out of `llms.txt` and loses its Markdown export → High severity; the fix is moving the file to the conventional path, never editing the glob.
...
- **Image assets** — every referenced image (`/assets/images/blog/YYYY/MM/<slug>/<file>`) actually exists in that post's asset folder under `docs/assets/images/blog/`; flag missing files.
- **Links** — internal links and image/asset references are **root-absolute with the `.md`/file extension** (`/meet/index.md` for pages, `/blog/posts/YYYY/MM/<slug>.md` for other posts, `/assets/images/blog/YYYY/MM/<slug>/<file>` for assets — also in raw HTML `src`); external links carry `{:target="_blank"}`. Flag relative internal links (`../../meet/...` — they break when the post moves at publish), bare pretty-URLs (`/meet/` — not validated by MkDocs), and obviously broken paths. **Release posts** (`Release` category) are the exception: links to versioned docs must be **absolute, version-pinned URLs including the domain** (`https://openvidu.io/X.Y/docs/...`, `/X.Y/meet/...`) for the announced version — flag `latest` or root-absolute `.md` links there.
