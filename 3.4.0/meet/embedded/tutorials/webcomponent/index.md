...
[Source code](https://github.com/OpenVidu/openvidu-meet-tutorials/tree/3.4.0/meet-webcomponent-basic)

This tutorial extends the [Direct Link tutorial](https://openvidu.io/3.4.0/meet/embedded/tutorials/direct-link/index.md) by integrating the **OpenVidu Meet WebComponent** directly into your application instead of using external links. It is built using **Node.js and Express** for the backend and plain **HTML/CSS/JavaScript** for the frontend.
...
The application uses the [OpenVidu Meet API](https://openvidu.io/3.4.0/meet/embedded/reference/rest-api/index.md) to create and delete rooms, and the [OpenVidu Meet WebComponent](https://openvidu.io/3.4.0/meet/embedded/reference/webcomponent/index.md) to embed the video call interface directly into the application.
...
This tutorial builds upon the [Direct Link tutorial](https://openvidu.io/3.4.0/meet/embedded/tutorials/direct-link/index.md), replacing external redirect links with an embedded OpenVidu Meet WebComponent. The backend remains identical, so we'll focus on the frontend modifications that enable WebComponent integration.

______________________________________________________________________

### Backend

The backend is identical to the [Direct Link tutorial](https://openvidu.io/3.4.0/meet/embedded/tutorials/direct-link/index.md). It provides the same three REST API endpoints:
...
For detailed backend documentation, please refer to the [Direct Link tutorial backend section](https://openvidu.io/3.4.0/meet/embedded/tutorials/direct-link/#backend).
...
1. **Configure OpenVidu Meet for network access**: Start OpenVidu Meet by following the instructions in the [Accessing OpenVidu Meet from other computers or phones](https://openvidu.io/3.4.0/meet/deployment/local/#accessing-openvidu-meet-from-other-computers-or-phones) section.
...
If you have a production deployment of OpenVidu Meet (installed in a server following [deployment steps](https://openvidu.io/3.4.0/meet/deployment/basic/index.md) ), you can connect this tutorial to it by following these steps:
...
1. **Update the API key**: Ensure the `OV_MEET_API_KEY` environment variable in the `.env` file matches the API key configured in your production deployment. See [Generate an API Key](https://openvidu.io/3.4.0/meet/embedded/reference/rest-api/#generate-an-api-key) section to learn how to obtain it.
