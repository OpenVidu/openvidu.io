...
- [**Live Captions**](https://openvidu.io/3.4.0/docs/ai/live-captions/index.md): transcribe the audio tracks of your Rooms in real time with great accuracy and display the results as live captions in your frontend.

## Enable the agent and configure AI services

### 1. SSH into an OpenVidu Node and go to configuration folder

Depending on your [OpenVidu deployment type](https://openvidu.io/3.4.0/docs/self-hosting/deployment-types/index.md):

If you are using [OpenVidu Local (Development)](https://openvidu.io/3.4.0/docs/self-hosting/deployment-types/#openvidu-local-development), simply navigate to the configuration folder of the project:
...
If you are using [OpenVidu Single Node](https://openvidu.io/3.4.0/docs/self-hosting/deployment-types/#openvidu-single-node), SSH into the only OpenVidu node and navigate to:

```bash
cd /opt/openvidu/config
```

If you are using [OpenVidu Elastic](https://openvidu.io/3.4.0/docs/self-hosting/deployment-types/#openvidu-elastic), SSH into the only Master Node and navigate to:

```bash
cd /opt/openvidu/config/cluster/media_node
```

If you are using [OpenVidu High Availability](https://openvidu.io/3.4.0/docs/self-hosting/deployment-types/#openvidu-high-availability), SSH into any of your Master Nodes (doesn't matter which one) and navigate to:
...
- **Live Captions**: see [Live Captions service](https://openvidu.io/3.4.0/docs/ai/live-captions/#how-to-enable-live-captions-service-in-your-openvidu-deployment).
...
[agent-speech-processing.yaml](https://github.com/OpenVidu/openvidu-agents/blob/3.4.0/speech-processing/agent-speech-processing.yaml)
