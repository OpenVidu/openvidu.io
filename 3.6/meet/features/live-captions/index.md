...
>
> Live Captions are **not available** in local Meet deployments. You must use either the [OpenVidu Local deployment](https://openvidu.io/3.6/docs/self-hosting/local/index.md) or a [OpenVidu production deployment](https://openvidu.io/3.6/docs/self-hosting/deployment-types/index.md) to enable this feature.
...
>
> For a more advanced setup, consider using a cloud-based provider. See [Cloud providers](https://openvidu.io/3.6/docs/ai/live-captions/#cloud-providers) for more information.

> **Default language is English**
>
> The Speech Processing Agent uses **English** for speech-to-text transcription by default. To use a different language, you must configure a different Vosk model. See [Vosk models configuration](https://openvidu.io/3.6/docs/ai/live-captions/#vosk) for details on changing the language model.
...
Captions are enabled by default when a room is created, whether through the UI or the [REST API](https://openvidu.io/3.6/meet/embedded/reference/api.html#/operations/createRoom). This behavior can be overridden to enable or disable captions on a per-room basis.
