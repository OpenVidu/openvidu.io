...
[Source code](https://github.com/OpenVidu/openvidu-tutorials/tree/3.6.1/openvidu-components-angular/openvidu-custom-chat-panel)
...
1. `messages` array that stores the chat messages.
1. `onTokenRequested` method that fires when the VideoconferenceComponent requests a token to connect to the OpenVidu Room.
1. `onRoomCreated` method that handles the creation of a new room and sets up an event listener for receiving data.
1. Event listener for the RoomEvent.DataReceived event that listens to chat messages.
1. `send` method that sends a chat message.
