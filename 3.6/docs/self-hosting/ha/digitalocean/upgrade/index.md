...
In DigitalOcean environments, we recommend upgrading by redeploying [OpenVidu High Availability DigitalOcean](https://openvidu.io/3.6/docs/self-hosting/ha/digitalocean/install/index.md) stack using the latest version. This approach ensures that all components are updated accurately and consistently, as DigitalOcean terraform files and related configurations may vary between releases. Redeploying guarantees that all necessary changes are properly applied.
...
1. After upgrading all your Master Nodes, **you need to delete the Media Nodes** to apply the changes to run the Media Nodes with the new version. Go to your DigitalOcean web, Droplets tab, select the Media Nodes instances and terminate them. The fixed autoscale pool will automatically launch new Media Nodes with the updated configuration.
