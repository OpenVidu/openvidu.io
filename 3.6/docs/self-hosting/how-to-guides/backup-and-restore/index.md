...
1. Only Snapshots data offered by external services used for S3 or MongoDB.
...
| - You are using S3 (MinIO) and MongoDB internally with your deployment. - You just want to backup OpenVidu Platform and OpenVidu Meet data. - You want to move your S3/MinIO and MongoDB data a new OpenVidu installation                                                                                                               | [Method 3: Only backup data from MinIO/S3 and MongoDB](#method-3-only-backup-data-from-minios3-and-mongodb)                                                            |
| - You are using external S3 and MongoDB services in your deployment. - You just want to backup OpenVidu Platform and OpenVidu Meet data. - You want to reduce operational overhead by delegating data backups.                                                                                                                          | [Method 4: Only Snapshots data offered by external services used for S3 or MongoDB](#method-4-only-snapshots-data-offered-by-external-services-used-for-s3-or-mongodb) |
...
1. Follow your cloud provider's snapshot workflow:
   - AWS: Check [EC2 backup and recovery with snapshots and AMIs](https://docs.aws.amazon.com/en_us/prescriptive-guidance/latest/backup-recovery/ec2-backup.html).
...
1. Automate snapshots using your provider's scheduler (AWS Backup, Azure Automation, Cloud Scheduler) for consistent recovery points.
1. Restore by cloning a new node from the snapshot/AMI, updating DNS or load balancer targets.
...
1. **Check that the following parameters are correctly configured**:
   ...
   **Elastic**
   ...
   - In case you need to reinstall Media Nodes, follow [this command](https://openvidu.io/3.6/docs/self-hosting/elastic/on-premises/install/#non-interactive-installation)
   ...
   **High Availability**
   ...
   - In case you need to reinstall Media Nodes, follow [this command](https://openvidu.io/3.6/docs/self-hosting/ha/on-premises/install-dlb/#non-interactive-installation)
...
1. **In AWS, Azure, or GCP deployments**: Ensure if you followed [Method 1](#method-1-direct-snapshots-of-openvidu-nodes-via-your-infrastructure-provider) that the restored VMs have the correct security groups, IAM roles, and network configurations.
