...
If none of these scenarios apply to you and you prefer to use the default internal Caddy server, please refer to the [official installation guides](https://openvidu.io/3.6/docs/self-hosting/deployment-types/index.md).
...
You can follow the same rule ports of the [Single Node Deployment](https://openvidu.io/3.6/docs/self-hosting/single-node/on-premises/install/#port-rules) but some ports are used by the proxy server and others are not needed. The inbound rules for the OpenVidu proxy would be as follows:
...
If you want to force all traffic including WebRTC to go through the external proxy, check the [Force media traffic through port 443](https://openvidu.io/3.6/docs/self-hosting/how-to-guides/force-single-port/index.md) guide.

**Elastic**

> **Note**
>
> The Elastic deployment with an external proxy is based on the same instructions as the [Elastic Deployment](https://openvidu.io/3.6/docs/self-hosting/elastic/on-premises/install/index.md), but with some modifications to the installation command and port rules. We recommend you to read the [Elastic Deployment](https://openvidu.io/3.6/docs/self-hosting/elastic/on-premises/install/index.md) guide before proceeding with this guide to have a better understanding of the deployment.
...
- An additional machine for the proxy server is recommended. Alternatively, you can use the same machine as the Master Node, but be aware that the proxy server will consume resources. Note that [some ports will be used by OpenVidu](https://openvidu.io/3.6/docs/self-hosting/elastic/on-premises/install/#port-rules-master-node), except for the ports utilized by the proxy server (TCP 80, 443, and 1935).
...
You can follow the same rule ports of the Elastic Deployment for the [Master Node](https://openvidu.io/3.6/docs/self-hosting/elastic/on-premises/install/#port-rules-master-node) and for the [Media Nodes](https://openvidu.io/3.6/docs/self-hosting/elastic/on-premises/install/#port-rules-media-nodes) but some ports are used by the proxy server and others are not needed. The inbound rules for the OpenVidu proxy would be as follows:
...
The High Availability deployment already has a way to configure an external proxy (described as a Network Load Balancer), which is explained [in this section](https://openvidu.io/3.6/docs/self-hosting/ha/on-premises/install-nlb/index.md).
