...
[Enable the Speech Processing agent](https://openvidu.io/3.6/docs/ai/openvidu-agents/speech-processing-agent/#enable-the-agent-and-configure-ai-services)

You configure the Live Captions service by setting up the following properties when [modifying file `agent-speech-processing.yaml`](https://openvidu.io/3.6/docs/ai/openvidu-agents/speech-processing-agent/#2-modify-file-agent-speech-processingyaml):

[agent-speech-processing.yaml](https://github.com/OpenVidu/openvidu-agents/blob/3.6.1/speech-processing/agent-speech-processing.yaml)
...
- **Manual processing**: the agent will connect to new Rooms only when your application dictates it with [explicit agent dispatch](https://openvidu.io/3.6/docs/ai/openvidu-agents/agent-dispatch/#explicit-agent-dispatch). This allows you to toggle Live Captions service on demand for specific Rooms. Set YAML property `live_captions.processing` to `manual` in file [`agent-speech-processing.yaml`](#how-to-enable-live-captions-service-in-your-openvidu-deployment).

Learn more about [Automatic vs Manual processing](https://openvidu.io/3.6/docs/ai/openvidu-agents/agent-dispatch/index.md).

## How to receive Live Captions in your frontend application

Live Captions are received in your frontend application using the [Text Stream API](https://docs.livekit.io/transport/data/text-streams/#handling-incoming-streams) of LiveKit client SDKs. You must specifically subscribe to the Room topic **`lk.transcription`** to automatically receive transcription events. For example, in JavaScript:
...
> **Info**
>
> Refer to [LiveKit documentation](https://docs.livekit.io/agents/multimodality/text/#frontend-rendering) to see how to handle transcription events in other frontend platforms.
...
| ----------- | ------------------------ | -------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------- |
|             | `aws`                    | `docker.io/openvidu/agent-speech-processing-cloud:3.6.1` | Uses [Amazon Transcribe](https://aws.amazon.com/transcribe/)                                                                                                           |                 |
|             | `azure`                  | `docker.io/openvidu/agent-speech-processing-cloud:3.6.1` | Uses [Azure Speech service](https://learn.microsoft.com/azure/ai-services/speech-service/index-speech-to-text)                                                         |                 |
|             | `azure_openai`           | `docker.io/openvidu/agent-speech-processing-cloud:3.6.1` | Uses [Azure OpenAI](https://azure.microsoft.com/en-us/products/ai-services/openai-service/)                                                                            |                 |
|             | `google`                 | `docker.io/openvidu/agent-speech-processing-cloud:3.6.1` | Uses [Google Cloud Speech-to-Text](https://cloud.google.com/speech-to-text)                                                                                            |                 |
|             | `openai`                 | `docker.io/openvidu/agent-speech-processing-cloud:3.6.1` | Uses [OpenAI Speech to text](https://platform.openai.com/docs/guides/speech-to-text)                                                                                   |                 |
|             | `groq`                   | `docker.io/openvidu/agent-speech-processing-cloud:3.6.1` | Uses [Groq Speech](https://console.groq.com/docs/speech-to-text)                                                                                                       |                 |
|             | `deepgram`               | `docker.io/openvidu/agent-speech-processing-cloud:3.6.1` | Uses [Deepgram Speech-to-Text API](https://deepgram.com/product/speech-to-text)                                                                                        |                 |
|             | `assemblyai`             | `docker.io/openvidu/agent-speech-processing-cloud:3.6.1` | Uses [AssemblyAI Speech-to-Text API](https://www.assemblyai.com/products/speech-to-text)                                                                               |                 |
|             | `fal`                    | `docker.io/openvidu/agent-speech-processing-cloud:3.6.1` | Uses [Fal Speech-to-Text API](https://docs.fal.ai/guides/convert-speech-to-text/)                                                                                      |                 |
|             | `clova`                  | `docker.io/openvidu/agent-speech-processing-cloud:3.6.1` | Uses [Naver Clova Speech Recognition](https://api.ncloud-docs.com/docs/en/ai-naver-clovaspeechrecognition-stt) . Specialized in Japanese, Korean and Chinese languages |                 |
|             | `speechmatics`           | `docker.io/openvidu/agent-speech-processing-cloud:3.6.1` | Uses [Speechmatics Real-Time API](https://docs.speechmatics.com/introduction/rt-guide)                                                                                 |                 |
|             | `gladia`                 | `docker.io/openvidu/agent-speech-processing-cloud:3.6.1` | Uses [Gladia Speech-to-Text API](https://www.gladia.io/product/async-transcription)                                                                                    |                 |
|             | `sarvam`                 | `docker.io/openvidu/agent-speech-processing-cloud:3.6.1` | Uses [Sarvam Speech-to-Text API](https://docs.sarvam.ai/api-reference-docs/speech-to-text/transcribe) . Optimized for Indian languages                                 |                 |
|             | `mistralai`              | `docker.io/openvidu/agent-speech-processing-cloud:3.6.1` | Uses [Voxtral](https://mistral.ai/news/voxtral)                                                                                                                        |                 |
|             | `cartesia`               | `docker.io/openvidu/agent-speech-processing-cloud:3.6.1` | Uses [Cartesia Ink-Whisper](https://cartesia.ai/ink)                                                                                                                   |                 |
|             | `soniox`                 | `docker.io/openvidu/agent-speech-processing-cloud:3.6.1` | Uses [Soniox Speech-to-Text API](https://soniox.com/speech-to-text)                                                                                                    |                 |
|             | `nvidia`                 | `docker.io/openvidu/agent-speech-processing-cloud:3.6.1` | Uses [NVIDIA Riva ASR](https://docs.nvidia.com/deeplearning/riva/user-guide/docs/asr/asr-overview.html)                                                                |                 |
|             | `spitch`                 | `docker.io/openvidu/agent-speech-processing-cloud:3.6.1` | Uses [Spitch Speech To Text API](https://docs.spitch.app/api/speech/stt) . Specialized in African languages                                                            |                 |
|             | `elevenlabs`             | `docker.io/openvidu/agent-speech-processing-cloud:3.6.1` | Uses [ElevenLabs Speech To Text API](https://elevenlabs.io/speech-to-text)                                                                                             |                 |
|             | `simplismart`            | `docker.io/openvidu/agent-speech-processing-cloud:3.6.1` | Uses [Simplismart](https://simplismart.ai/)                                                                                                                            |                 |
...
1. In file [`agent-speech-processing.yaml`](#how-to-enable-live-captions-service-in-your-openvidu-deployment) set properties `docker_image` and `live_captions.provider` as follows:
   ...
   ```yaml
   docker_image: docker.io/openvidu/agent-speech-processing-vosk:3.6.1
   live_captions:
     provider: vosk
   ```

1. Configure additional Vosk settings under `live_captions.vosk`. Especially important is property `model` to select the desired language model.
...
The default Docker image `docker.io/openvidu/agent-speech-processing-vosk:3.6.1` comes with small-sized models for multiple languages pre-installed. You can build your own Docker image with exactly the models you need (<https://alphacephei.com/vosk/models>).

1. Create this two-line Dockerfile:

   ```Dockerfile
   FROM docker.io/openvidu/agent-speech-processing-vosk-base:3.6.1
   COPY --chown=appuser:appuser vosk-models /app/vosk-models
   ```

1. Download and unzip the [desired Vosk models](https://alphacephei.com/vosk/models) into a local folder `vosk-models`. The folder structure should be like this:
...
1. Build the Docker image and push it to your remote Docker repository:

   ```bash
   docker build -f Dockerfile -t <YOUR_DOCKERHUB_ACCOUNT>/agent-speech-processing-vosk:CUSTOM_TAG .
   ```

1. Update your OpenVidu deployment by setting property `docker_image` in file [`agent-speech-processing.yaml`](#how-to-enable-live-captions-service-in-your-openvidu-deployment):
...
[sherpa-onnx](https://k2-fsa.github.io/sherpa/onnx/index.html) is an offline, cutting-edge speech recognition toolkit that offers competitive accuracy and performance.
...
1. In file [`agent-speech-processing.yaml`](#how-to-enable-live-captions-service-in-your-openvidu-deployment) set properties `docker_image` and `live_captions.provider` as follows:
   ...
   ```yaml
   docker_image: docker.io/openvidu/agent-speech-processing-sherpa:3.6.1
   live_captions:
     provider: sherpa
   ```

1. Configure additional Sherpa settings under `live_captions.sherpa`. Especially important is property `model` to select the desired language model.
...
The default Docker image `docker.io/openvidu/agent-speech-processing-sherpa:3.6.1` comes with small-sized models for multiple languages pre-installed. You can build your own Docker image with exactly the models you need ([sherpa-onnx ASR models](https://github.com/k2-fsa/sherpa-onnx/releases/tag/asr-models) ).

1. Create this two-line Dockerfile:

   ```Dockerfile
   FROM docker.io/openvidu/agent-speech-processing-sherpa-base:3.6.1
   ...
   >
   > - `FROM docker.io/openvidu/agent-speech-processing-sherpa-cuda11-base:3.6.1` if your GPU is compatible only with CUDA 11.
   > - `FROM docker.io/openvidu/agent-speech-processing-sherpa-cuda12-base:3.6.1` if your GPU is compatible with CUDA 12 or higher.

1. Download and unzip the [desired sherpa-onnx streaming models](https://github.com/k2-fsa/sherpa-onnx/releases/tag/asr-models) into a local folder `sherpa-onnx-streaming-models`. The folder structure should be like this:
...
- The OpenVidu node running the Speech Processing agent must have a compatible **NVIDIA GPU (>= CUDA 11.0)** with the appropriate [NVIDIA Container Toolkit](https://docs.nvidia.com/datacenter/cloud-native/container-toolkit/latest/install-guide.html) installed.

- The OpenVidu node must have enough disk and memory to handle the large Docker containers with GPU support (+4GB).

  > **Info**
  >
  > - Check your Nvidia GPU compatibility by running the following command, which must show a table with GPU info: `nvidia-smi`
...
```yaml
docker_image: docker.io/openvidu/agent-speech-processing-sherpa-cuda12:3.6.1 #(1)!
...
1. This property is necessary for the agent container to launch.
1. These Docker options are necessary to enable GPU access for the agent container.
1. This property is necessary to tell the agent to use the GPU-enabled runtime of Sherpa ONNX instead of CPU.
...
Check out the [Live Captions tutorial](https://openvidu.io/3.6/docs/tutorials/ai-services/openvidu-live-captions/index.md) for a complete example.
