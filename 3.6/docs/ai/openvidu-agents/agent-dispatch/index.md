...
To configure automatic dispatch for an agent's AI service, set this property in your `agent-AGENT_NAME.yaml` file:
...
> **Tip**
>
> For example, for the [Live Captions](https://openvidu.io/3.6/docs/ai/live-captions/index.md) service:
>
> ```yaml
> live_captions:
>     processing: automatic
...
Agents configured with `processing: automatic` in one of their AI services will immediately join new Rooms and will start processing media tracks as soon as possible. This *"as soon as possible"* moment can vary depending on the type of agent, the AI service that is providing, and its configuration.

> **Automatic dispatch is useful when the same AI service needs to be present in all Rooms, at all times.**

## Explicit agent dispatch

To configure explicit dispatch for an agent's AI service, set this property in your `agent-AGENT_NAME.yaml` file:
...
> **Explicit dispatch is useful when you need fine control over which Rooms the agent should process, and when.**
...
1. Consume the Agent Dispatch API directly:
   ...
   > You need as `VALID_AUTHORIZATION_TOKEN` a token with `room` and `roomAdmin` permissions. Visit LiveKit docs: [Creating a token](https://docs.livekit.io/frontends/build/authentication/#alternative-manual-token-generation)
...
> **Notes about the Agent Dispatch API**
>
...
To create a Participant's token with Agent dispatch, you just need to include in the token the proper `RoomConfiguration` options, specifically the **`agents`** property. Visit [LiveKit docs](https://docs.livekit.io/frontends/reference/tokens-grants/#room-configuration) to learn how.
