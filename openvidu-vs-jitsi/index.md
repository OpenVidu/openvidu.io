...
|                       | **OpenVidu**                                                                                                                                                                                                                                                 | **Jitsi**                                                                                                                                                                                                      |
| --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Components to operate | A [fork of LiveKit](https://openvidu.io/openvidu-vs-livekit/index.md), optionally with mediasoup as the media engine**PRO** and [OpenVidu Meet](https://openvidu.io/latest/meet/index.md) as a web frontend — one integrated stack                          | Prosody (XMPP signaling), Jicofo (conference focus), Jitsi Videobridge (SFU, Java), and the Jitsi Meet web frontend — four separately-versioned components, a single bundle                                    |
| License               | Apache 2.0**COMMUNITY** / commercial**PRO**                                                                                                                                                                                                                  | Apache 2.0                                                                                                                                                                                                     |
| Recording/streaming   | [Egress bundled by default](https://openvidu.io/latest/docs/reference/egress/index.md), no extra hardware sizing                                                                                                                                                | Jibri, can be deployed as an additional bundle                                                                                                                                                                 |
| Horizontal scaling    | [Elastic & HA modes](https://openvidu.io/latest/docs/self-hosting/production-ready/scalability/index.md)**PRO**, one product to configure & one-click deploy for 5 cloud providers                                                                              | Octo relays media across an existing pool of Videobridges; actually growing that pool needs a separate `jitsi-autoscaler` service, supporting only Oracle OCI, DigitalOcean or a custom provider you implement |
| Admin dashboard       | [OpenVidu Dashboard](https://openvidu.io/latest/docs/self-hosting/production-ready/observability/openvidu-dashboard/index.md)**COMMUNITY**, per-room and per-participant views**PRO**                                                                           | None bundled                                                                                                                                                                                                   |
| Ready-to-use app      | [OpenVidu Meet](https://openvidu.io/latest/meet/index.md), embeddable via [iframe](https://openvidu.io/latest/meet/embedded/step-by-step-guide/#use-an-iframe) or [web component](https://openvidu.io/latest/meet/embedded/step-by-step-guide/#use-the-web-component) | Jitsi Meet, embeddable via iframe, lib-jitsi-meet, or native SDKs                                                                                                                                              |
| Hosted/cloud option   | None — [self-hosted](https://openvidu.io/latest/docs/self-hosting/deployment-types/index.md) only, on your own infrastructure. One-click deploy for [5 cloud providers](https://openvidu.io/latest/docs/self-hosting/single-node/index.md)                         | [Jitsi as a Service](https://jaas.8x8.vc/) (8x8), MAU-priced                                                                                                                                                   |
...
This is the sharpest operational difference between the two projects. OpenVidu **COMMUNITY** ships [Egress](https://openvidu.io/latest/docs/reference/egress/index.md) wired up by default, writing to S3-compatible storage out of the box. Jitsi's Jibri is the service responsible for recordings, but storing and serving the resulting files is left to you.

## Scaling: Elastic/HA vs Octo plus a separate autoscaler

Both projects scale horizontally, but the operational shape differs. OpenVidu's [Elastic and HA modes](https://openvidu.io/latest/docs/self-hosting/production-ready/scalability/index.md) are delivered as a configured product: you pick a mode and OpenVidu's automated deployments handle the rest.
...
OpenVidu's approach is the LiveKit-compatible SDK set — 8 client SDKs including native iOS and Android — plus [OpenVidu Meet's own embedding path](https://openvidu.io/latest/meet/embedded/intro/index.md) for teams that want the finished app experience inside their product rather than building a custom UI from primitives.
...
[Start with a tutorial](https://openvidu.io/latest/docs/tutorials/application-server/index.md) [Evaluating a raw SFU instead?](https://openvidu.io/openvidu-vs-janus/index.md)
