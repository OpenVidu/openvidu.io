...
| `leave-redirect-url` | URL to redirect to when leaving OpenVidu Meet. Redirection happens when the participant dismisses the post-meeting, lobby, error or recording screen, right after the **`meetingClosed` event** fires. | No |
