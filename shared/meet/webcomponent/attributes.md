...
| `leave-redirect-url` | URL to redirect to when leaving OpenVidu Meet. Redirection happens when the participant dismisses the post-meeting, join, error or recording screen, right after the **`viewClosed` event** fires. | No |
