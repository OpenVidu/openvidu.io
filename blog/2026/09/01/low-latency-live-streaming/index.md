...
[OpenVidu Platform](https://openvidu.io/latest/docs/index.md)'s Ingress module exposes a WHIP endpoint out of the box, so an encoder can push straight into a Room over WebRTC. You can even skip transcoding entirely when you want to shave off every extra millisecond. From there, every participant in the Room receives that stream over native WebRTC too. See the [stream ingestion guide](https://openvidu.io/latest/docs/build-your-app/common-operations/#stream-ingestion) for how to wire a WHIP source into your own app.
...
However, if you want to try it by yourself now, we have you covered. This article explains how to ingest video through WHIP in your app 👉 **[Add WHIP-based low-latency ingest to your app](https://openvidu.io/latest/docs/build-your-app/common-operations/#stream-ingestion)**: the fastest way to feel the difference is to push a real encoder into a Room and watch the delay disappear.

To go further:

- [OpenVidu Platform](https://openvidu.io/latest/docs/index.md) — the low-level SDKs and APIs for building your own interactive streaming experience.
- [How to scale video conferencing architecture](https://openvidu.io/blog/2026/06/02/scalability-in-videoconferencing-systems/index.md) — what changes once a single low-latency session needs to serve far more than one media server can handle.
- [Connectivity Resilience and Security in WebRTC Deployments](https://openvidu.io/blog/2026/06/09/turn-key-considerations/index.md) — why the same NAT and firewall problems that affect video calls apply just as much to low latency ingest.
