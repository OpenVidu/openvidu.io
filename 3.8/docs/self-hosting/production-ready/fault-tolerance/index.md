...
The extent of fault tolerance depends on the [OpenVidu deployment type](https://openvidu.io/3.8/docs/self-hosting/deployment-types/index.md):
...
In the event of a Media Node failure, there are [3 services](https://openvidu.io/3.8/docs/self-hosting/deployment-types/#media-node-services) affected with the following behaviors:

- Active [Rooms](https://docs.livekit.io/intro/basics/rooms-participants-tracks/) hosted by the failed Media Node will suffer a temporary interruption of about 5 seconds (this is the time the clients take to realize the Media Node has crashed). After that time has elapsed, the Room will be automatically reconstructed in a healthy Media Node. Every participant and track will be recreated and the Room will be fully operational again.
- Active [Egress](https://openvidu.io/3.8/docs/reference/egress/index.md) hosted by the failed Media Node will be interrupted. If the node's disk is still accessible, egress output files can still be recovered. See [Recovering Egress from node failures](#recovering-egress-from-node-failures).
- Active [Ingress](https://openvidu.io/3.8/docs/reference/ingress/index.md) hosted by the failed Media Node will be interrupted. The participants of the Room will receive the proper [events](https://openvidu.io/3.8/docs/reference/client-sdk/#room-events) indicating the Ingress participant has left the Room: `TrackUnpublished` and `ParticipantDisconnected`. Some popular tools for streaming such as OBS Studio will automatically try to reconnect the stream when they detect a connection loss, so in this case interruption will be minimal and the Ingress tracks will be restored on their own on a healthy Media Node.

## Fault tolerance in OpenVidu High Availability

OpenVidu High Availability delivers the highest possible degree of fault tolerance. This is achieved by running all of the [services in the Master Nodes and the Media Nodes](https://openvidu.io/3.8/docs/self-hosting/deployment-types/#node-services) in their **High Availability** flavour.
...
If **one** Master Node fails, the service won't be affected. Some users may trigger [event](https://openvidu.io/3.8/docs/reference/client-sdk/#room-events) `Reconnecting` closely followed by `Reconnected`, but the service will remain fully operational.

When two or more Master Nodes fail simultaneously, there can be some degradation of the service:

- If **two** Master Nodes fail, the service will still be operational for the most part. Only active [Egress](https://openvidu.io/3.8/docs/reference/egress/index.md) might be affected, as they won't be stored in the Minio storage. See [Recovering Egress from node failures](#recovering-egress-from-node-failures).
...
[Egress](https://openvidu.io/3.8/docs/reference/egress/index.md) processes can be affected by the crash of a Master Node or a Media Node. To recover Egress from...

### From Master Node failures

This only applies to OpenVidu High Availability

If 2 Master Nodes crash, the Egress process won't be able to use the Minio storage. This has different consequences depending on the [configured outputs](https://openvidu.io/3.8/docs/reference/egress/#outputs) for your Egress process:
...
It is possible that if the crashed Egress had **MP4** as [configured output](https://openvidu.io/3.8/docs/reference/egress/#outputs) (which is an option available for [Room Composite](https://openvidu.io/3.8/docs/reference/egress/#egress-types) and [Track Composite](https://openvidu.io/3.8/docs/reference/egress/#egress-types)) the recovered file may not be directly playable and it may require a repair process.

## Need specific uptime or SLA guarantees?
