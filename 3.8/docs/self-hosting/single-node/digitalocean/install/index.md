...
The [minimum inbound ports to allow](https://openvidu.io/3.8/docs/self-hosting/single-node/on-premises/install/#port-rules) must be included in the Firewall rules.
...
1. Follow the [On-Premises install instructions](https://openvidu.io/3.8/docs/self-hosting/single-node/on-premises/install/#guided-installation) to install OpenVidu on the instance.

______________________________________________________________________

### 4. Administration and upgrade

- For administration of this OpenVidu Single Node deployment, see the [On-Premises administration section](https://openvidu.io/3.8/docs/self-hosting/single-node/on-premises/admin/index.md).
- To upgrade OpenVidu, see the [Upgrade section](https://openvidu.io/3.8/docs/self-hosting/single-node/upgrade/index.md).
...
1. You will see logs appear in the terraform apply execution console. Wait for it to finish and display `Apply Complete!`. Now go to [Space Object Storage](https://cloud.digitalocean.com/spaces) and wait for the ssh key to appear in the bucket you have configured (`openvidu_ssh_key_sn.pem` for COMMUNITY, `openvidu_ssh_key_snpro.pem` for PRO).
...
1. Give the SSH Key the necessary permissions for it to work. Replace `<SSH_KEY_FILE>` below with `openvidu_ssh_key_sn.pem` (COMMUNITY) or `openvidu_ssh_key_snpro.pem` (PRO).
...
When your **OPENVIDU_URL** is reachable, it means that everything has gone well. Now you can check the [Administration](https://openvidu.io/3.8/docs/self-hosting/single-node/digitalocean/admin/index.md) section to learn how to manage your deployment.
