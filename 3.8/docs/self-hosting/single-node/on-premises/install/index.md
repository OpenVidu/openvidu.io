...
This section contains instructions for deploying a production-ready OpenVidu Single Node COMMUNITY deployment on-premises. It is based on Docker and Docker Compose, which automatically configure all necessary services for OpenVidu to work properly.
...
1. Execute the following commands to clean the iptables rules, accept all inputs and deactivate iptables at start:
...
1. Execute the following commands to add the required firewall rules:
...
- In the argument `--enabled-modules`, you can enable the modules you want to deploy. You can enable `openviduMeet` [OpenVidu Meet service](https://openvidu.io/3.8/meet/index.md) and `observability` (Grafana stack).
...
This installation method is targeted to advanced users
