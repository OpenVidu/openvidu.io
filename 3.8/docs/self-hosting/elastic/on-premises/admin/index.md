...
You can also review your logs using the Grafana dashboard provided with OpenVidu. To access it, go to https://\<your-domain.com>/grafana{:target="*blank"} and use the credentials located in `/opt/openvidu/.env` to log in. Once inside, navigate to the* "Home" *section, select* "Dashboard"\_, and then click on:
...
To add a new Media Node, simply spin up a new VM and run the OpenVidu installer script to integrate it into the existing cluster. Run the [installation command](https://openvidu.io/3.8/docs/self-hosting/elastic/on-premises/install/#media-node) on the new Media Node.
