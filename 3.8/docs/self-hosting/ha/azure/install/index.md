...
1. Inside the Virtual Machine resource, click on *"Connect"* 🡒 *"Connect via Bastion"*. Connect via bastion
1. Click on *"Deploy Bastion"* button. Deploy bastion
1. Change **Authentication Type** to *"SSH Private Key from Local File"*, set the same username used when you deployed OpenVidu, and select the local file corresponding to the SSH Private Key of the SSH Key used when deploying. Bastion parameters
