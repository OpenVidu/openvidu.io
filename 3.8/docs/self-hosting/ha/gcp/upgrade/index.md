...
In Google Cloud Platform environments, we recommend upgrading by redeploying the [OpenVidu High Availability Google Cloud Platform](https://openvidu.io/3.8/docs/self-hosting/ha/gcp/install/index.md) stack using the latest version. This approach ensures that all components are updated accurately and consistently, as Google Cloud Platform terraform files and related configurations may vary between releases. Redeploying guarantees that all necessary changes are properly applied.
...
1. **Repeat the steps 1 to 4 in all the Master Nodes of your deployment.** This is important because the Master Nodes need to be running the same version of OpenVidu.

1. After upgrading all your Master Nodes, **you need to delete the Media Nodes** to apply the changes. Go to your Google Cloud Platform Console, Compute Engine instances, selecting the Media Nodes instances, and terminating them. The MIG will automatically launch new Media Nodes with the updated configuration.
