...
For Oracle Cloud Infrastructure environments, we recommend upgrading by redeploying the [OpenVidu High Availability Oracle Cloud Infrastructure](https://openvidu.io/3.8/docs/self-hosting/ha/oracle/install/index.md) stack using the latest version. This approach ensures that all components are updated accurately and consistently, since Oracle Cloud Infrastructure Terraform files and related configurations may differ between releases. Redeploying guarantees that all necessary changes are properly applied.
...
## Upgrading OpenVidu High Availability on Oracle Cloud Infrastructure
...
1. This will run an update script that guides you from your currently installed version to the target one. The output will begin with the following:
...
1. **Confirm each version upgrade.** You can jump straight to the latest release regardless of how far behind you are — the updater will automatically apply every intermediate upgrade in sequence.
...
1. **Repeat steps 1 to 4 on each of the remaining Master Nodes of your deployment.** This is important because all 4 Master Nodes need to be running the same version of OpenVidu.

1. After the final intermediate upgrade completes on all 4 Master Nodes, **you need to terminate the Media Nodes** to apply the changes. Go to the OCI Console, navigate to the **Compute → Instance Pools** section, open `<STACK_NAME>-media-pool`, and terminate the existing Media Node instances. The Instance Pool will automatically launch new Media Nodes with the updated configuration.

1. Once the Media Nodes are up and running, restart OpenVidu High Availability by running the following command **on every Master Node**:
...
Once the upgrade is complete, a backup of the previous version will be available in the `/opt/openvidu/backups` directory of each Master Node. The backup contains only the configuration files that were changed during the upgrade.

To roll back to the previous version, copy the files from the backup back to the OpenVidu directory **on each Master Node** using the following commands:
...
- Always upgrade all nodes in your OpenVidu High Availability deployment. Running mismatched versions across Master Nodes may cause compatibility issues.
...
- Remove unused images and containers to free up disk space. For example, once the upgrade is complete and OpenVidu is running, you can remove old images with the following command on each Master Node:
