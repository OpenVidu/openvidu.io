...
1. Consume the Agent Dispatch API directly:
   ...
   > You need as `VALID_AUTHORIZATION_TOKEN` a token with `room` and `roomAdmin` permissions. See [Generating a token](https://openvidu.io/3.8/docs/reference/access-tokens/#generating-a-token)
...
To create a Participant's token with Agent dispatch, you just need to include in the token the proper `RoomConfiguration` options, specifically the **`agents`** property. See the [`roomConfig`](https://openvidu.io/3.8/docs/reference/access-tokens/#token-claims) token claim.
