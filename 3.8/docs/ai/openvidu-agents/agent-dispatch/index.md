...
For example, for the [Live Captions](https://openvidu.io/3.8/docs/ai/live-captions/index.md) service:
...
1. Consume the Agent Dispatch API directly:
   ...
   > You need as `VALID_AUTHORIZATION_TOKEN` a token with `room` and `roomAdmin` permissions. Visit LiveKit docs: [Creating a token](https://docs.livekit.io/home/get-started/authentication/#creating-a-token)
...
To create a Participant's token with Agent dispatch, you just need to include in the token the proper `RoomConfiguration` options, specifically the **`agents`** property. Visit [LiveKit docs](https://docs.livekit.io/home/get-started/authentication/#room-configuration) to learn how.
