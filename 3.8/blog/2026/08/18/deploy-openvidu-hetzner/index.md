...
The [full port table](https://openvidu.io/3.8/docs/self-hosting/single-node/on-premises/install/#port-rules) in the docs lists three more **optional** ports. This guide skips them; each is one more rule away if the deployment ever needs it:
...
- **The OpenVidu Platform API**, based on LiveKit. The `LIVEKIT_URL`, `LIVEKIT_API_KEY` and `LIVEKIT_API_SECRET` values grepped above are what an application uses to create rooms and tokens programmatically, with [any LiveKit SDK](https://openvidu.io/3.8/docs/developing-your-openvidu-app/index.md).
- **The OpenVidu Meet API**, a higher-level [REST API](https://openvidu.io/3.8/meet/embedded/reference/rest-api/index.md) to manage rooms, recordings and users without touching the LiveKit layer. Generate its API key from the console's **Embedded** section and authenticate requests with the `X-API-KEY` header.
...
This post took the shortest honest path, which means it skipped options you might need. The [on-premises installation guide](https://openvidu.io/3.8/docs/self-hosting/single-node/on-premises/install/index.md) covers all of them: custom and ZeroSSL certificates, running behind your own proxy, plain Docker Compose installation, and a [non-interactive mode](https://openvidu.io/3.8/docs/self-hosting/single-node/on-premises/install/#non-interactive-installation) to bake the whole install into cloud-init or Terraform.

And a single node is only the first of the [deployment types](https://openvidu.io/3.8/docs/self-hosting/deployment-types/index.md). When one machine stops being enough, [OpenVidu Elastic](https://openvidu.io/3.8/docs/self-hosting/elastic/index.md) scales media nodes with demand and [OpenVidu High Availability](https://openvidu.io/3.8/docs/self-hosting/ha/index.md) removes the single points of failure. The single node deployed here uses the same configuration model, so nothing learned today gets thrown away.

If you would like to explore this further, two good next steps could be:

- Compare the [deployment options](https://openvidu.io/3.8/docs/self-hosting/deployment-types/index.md) that fit a real workload, from this single node to elastic and HA setups.
- If what you actually need is video calls inside your own product, look at [embedding OpenVidu Meet](https://openvidu.io/3.8/meet/embedded/intro/index.md) with your deployment's API key, or build directly on [OpenVidu Platform](https://openvidu.io/3.8/docs/developing-your-openvidu-app/index.md) with the LiveKit SDKs. You already have everything both require.
