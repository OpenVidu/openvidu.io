...
If gesture control isn't what you need but self-hosting your own video infrastructure is, that's exactly what **[OpenVidu](https://openvidu.io/3.8/docs/index.md)** is for: the same LiveKit-compatible core you just used, wrapped in a production-ready platform you can run anywhere, from a quick local Docker Compose install to a highly available cluster. The [self-hosting docs](https://openvidu.io/3.8/docs/self-hosting/local/index.md) are the natural next step.
