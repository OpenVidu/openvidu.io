...
We know that pain, which is why every OpenVidu deployment ships with a full [observability stack](https://openvidu.io/3.8/docs/self-hosting/production-ready/observability/index.md) (Grafana, Prometheus, Loki) so our users can see what their media servers are actually doing. Turning those logs and metrics into a diagnosis, though, still takes a human who knows where to look.
...
**What we broke:** recordings are refused, but for a sneaky reason. OpenVidu's egress runs an admission check before it accepts a recording: it only takes the job if the node has enough spare CPU for it ([documented here](https://openvidu.io/3.8/docs/troubleshooting/recording/#cpu-exhausted)). We set the per-recording CPU *cost* in `egress.yaml` to an absurd `100`, far more than the node's 16 cores, so the check can never pass and every recording is rejected with a *"not enough CPU"* error, even though the fake VM is basically idle.
...
We score each run in the usual AIOps terms, the stages an operator walks through when handling an incident:

- **Detection** (doesn't apply here): noticing that something is wrong. We always hand the agent a complaint, so it never has to discover the problem itself.
- **Localization**: pinning the failing component or subsystem, without necessarily knowing why.
- **Analysis** (root cause): going further and identifying the actual cause, the bad `cpu_cost` value, say, not just "recordings are failing."
- **Mitigation** (doesn't apply here): fixing the problem on its own. Out of scope, since the agent can only read.

So the table scores each run at the highest stage it reached, **Localization** or **Analysis**.

| Fault                                | Signal                 | Bare MCP                  | With skill      |
| ------------------------------------ | ---------------------- | ------------------------- | --------------- |
| Blocked media ports (ICE)            | loud (metric + logs)   | 🔍 Localization           | 🔍 Localization |
| Network congestion                   | loud (metric)          | ⚠️ often blamed the users | 🔍 Localization |
| Redis down                           | loud but logs-only     | ✅ Analysis               | ✅ Analysis     |
| Ingress bad stream key               | logs-only but explicit | ✅ Analysis               | ✅ Analysis     |
| Recordings refused ("CPU exhausted") | logs-only but explicit | ❌ blamed the hardware    | ✅ Analysis     |

The two tricky faults are where the skill mattered most: on **congestion** it reliably placed the fault on the server rather than the callers, and on the **"CPU exhausted"** recording refusal it saw the nonsensical config value behind the scary error instead of recommending more hardware.

**Honest limits.** A few things this approach *cannot* do:

- **Some root causes are invisible to Grafana.** A firewall rule doesn't log itself: in the ICE fault, the most either session could do was localize the *effect* ("media unreachable, check the firewall / the node's network config") and point at the right area; neither can read the `iptables` rule it will never see. When a cause leaves no trace in the data, expect a confident guess, not the truth, so check the infrastructure directly, outside Grafana.
...
**What it builds.** A local **OpenVidu Single Node Community** deployment, the free edition, running inside a simulated VM, with the full [observability module](https://openvidu.io/3.8/docs/self-hosting/production-ready/observability/index.md) (Grafana + Prometheus + Loki). What makes this feel like a real deployment?
...
1. **Enable observability.** Add `observability` to `ENABLED_MODULES` in `openvidu.env` (with `GRAFANA_ADMIN_USERNAME`/`GRAFANA_ADMIN_PASSWORD`) and restart. See the [modules guide](https://openvidu.io/3.8/docs/self-hosting/how-to-guides/enable-disable-modules/index.md).
...
1. **(Optional) Give it the signal map:** install the [`openvidu-grafana-triage` skill](https://github.com/openvidu-labs/openvidu-grafana-mcp-lab/blob/v0.1.0/mcp/with-skill/.claude/skills/openvidu-grafana-triage/SKILL.md) into `.claude/skills/`. That's the "with-skill" arm from the experiment above.
