...
There are three ways to integrate video conferencing into your app with OpenVidu, and all of them run on the same self-hosted deployment. You can embed [OpenVidu Meet](/meet/embedded/intro/), a finished meeting UI, with one HTML tag. You can assemble your own meeting screen from [Angular Components](/docs/ui-components/angular-components/). Or you can go down to the [OpenVidu Platform](/docs/) SDKs and handle every audio and video track yourself.
...
| **Product**        | [OpenVidu Meet](https://openvidu.io/3.8/meet/index.md)                                            | [OpenVidu Platform](https://openvidu.io/3.8/docs/index.md)           | [OpenVidu Platform](https://openvidu.io/3.8/docs/index.md) |

None of this changes what you pay. Both products work in OpenVidu COMMUNITY and OpenVidu PRO, and the [pricing](https://openvidu.io/3.8/pricing/index.md) depends on the deployment, not on how you integrate.

## Before the code: one deployment, three example apps

Every snippet below is trimmed to the lines that carry the idea. The complete, runnable version lives in [**openvidu-integration-levels**](https://github.com/openvidu-labs/openvidu-integration-levels) , where each level is an **independent application** with its own backend, frontend and README. Clone it, pick a folder, and run two commands. Each section below links to the code it comes from.

The scenario is the same throughout: a **support desk** where an agent starts a call and a customer joins.

All three levels need an OpenVidu deployment, and one is enough for the three of them. [OpenVidu Local](https://openvidu.io/3.8/docs/self-hosting/local/index.md) brings up both products with Docker:
...
This is the fastest path. OpenVidu Meet is a complete video conferencing application, and [OpenVidu Meet Embedded](https://openvidu.io/3.8/meet/embedded/intro/index.md) puts that application inside yours. On the client you have three ways to show a room: a [direct link](https://openvidu.io/3.8/meet/embedded/reference/direct-link/index.md), an [iframe](https://openvidu.io/3.8/meet/embedded/reference/iframe/index.md) or the [`<openvidu-meet>` Web Component](https://openvidu.io/3.8/meet/embedded/reference/webcomponent/index.md). On the server you have a [REST API](https://openvidu.io/3.8/meet/embedded/reference/rest-api/index.md) to manage rooms, members, recordings and users, and [webhooks](https://openvidu.io/3.8/meet/embedded/reference/webhooks/index.md) to react to what happens in them.
...
From there the element talks to your app in both directions, and you use it like any other element in your template. It emits [events](https://openvidu.io/3.8/meet/embedded/reference/webcomponent/#events) when participants come and go, which you bind with the usual `(event)` syntax, and it accepts [commands](https://openvidu.io/3.8/meet/embedded/reference/webcomponent/#commands) so your own buttons can drive the meeting, which you call through a `viewChild` reference:
...
> **See it running**
>
> [**`1-meet-embedded/`**](https://github.com/openvidu-labs/openvidu-integration-levels/tree/main/1-meet-embedded) is this level as a standalone app: [`server.js`](https://github.com/openvidu-labs/openvidu-integration-levels/blob/main/1-meet-embedded/backend/server.js) creates the room, [`app.html`](https://github.com/openvidu-labs/openvidu-integration-levels/blob/main/1-meet-embedded/frontend/src/app/app.html) and [`app.ts`](https://github.com/openvidu-labs/openvidu-integration-levels/blob/main/1-meet-embedded/frontend/src/app/app.ts) embed it. Run it, hit **Start call**, and open the customer link in a second tab to see both sides of the meeting.
...
>
> Your use case is video conferencing (telehealth, e-learning, customer support, team collaboration), you want recording, chat and screen sharing without building them, and applying your colors to a proven UI is enough for your brand. This is the path we took in [Building a video-enabled CRM with an AI agent](https://openvidu.io/3.8/blog/2026/07/21/building-a-video-enabled-crm-with-an-ai-agent/index.md).

## Level 2: Angular Components

The second level is [Angular Components](https://openvidu.io/3.8/docs/ui-components/angular-components/index.md), the library we use to build OpenVidu Meet itself. It gives you a `<ov-videoconference>` element that renders a complete meeting, and lets you adapt, extend or replace any part of it. You get a working screen in minutes and then work on your customizations from there.

### Generate access tokens in your backend

The backend changes at this level. You are no longer talking to OpenVidu Meet but to OpenVidu directly, through the LiveKit-compatible server SDK, and the one thing your server must do is generate [access tokens](https://openvidu.io/3.8/docs/reference/access-tokens/index.md). An access token is a JWT signed with your API secret that states who the participant is and which room they may join:
...
The [Angular Components tutorials](https://openvidu.io/3.8/docs/tutorials/angular-components/index.md) walk through each one of them separately, from a custom toolbar to a custom layout, custom streams, custom panels and an admin dashboard, and the [reference](https://openvidu.io/3.8/docs/reference-docs/openvidu-components-angular/index.md) lists every component, directive and CSS variable you can reach.
...
At the bottom of the stack there is no UI at all, just a `Room` and its tracks. OpenVidu is a fork of LiveKit that keeps 100% API compatibility, so any [LiveKit client SDK](https://docs.livekit.io/reference/) works unchanged against your deployment. In the browser, that SDK is `livekit-client`, and the token server from Level 2 is all the backend you need:
...
- **Level 2** in React means the [React Components](https://openvidu.io/3.8/docs/ui-components/react-components/index.md) listed in our docs under UI Components. A `<LiveKitRoom>` with a `<VideoConference>` inside gets you a prebuilt meeting, and its hooks and contexts let you build your own.
- **Level 3** has tutorials for JavaScript, React, Angular, Vue, Electron, Ionic, Android and iOS on the client, and Node.js, Go, Ruby, Java, Python, Rust, PHP and .NET for the [token server](https://openvidu.io/3.8/docs/tutorials/application-server/index.md). Any client works with any server.
...
You do not have to pick one level for the whole product either. Both products run on the same deployment, so a telehealth platform can embed OpenVidu Meet for consultations and use the SDKs for a one-way waiting-room stream. The [Meet vs Platform comparison](https://openvidu.io/3.8/openvidu-meet-vs-openvidu-platform/index.md) has the side-by-side table, and [OpenVidu Meet vs OpenVidu Platform in 2026](https://openvidu.io/3.8/blog/2026/06/16/meet-vs-platform/index.md) goes deeper into why we split the two products in the first place.

## Need more than this?

**Clone the examples and run them against one deployment.** Everything in this post is in [openvidu-labs/openvidu-integration-levels](https://github.com/openvidu-labs/openvidu-integration-levels) : three independent applications, each with a README that takes you from an empty folder to a working meeting. Then go deeper:

- [OpenVidu Meet Embedded step-by-step guide](https://openvidu.io/3.8/meet/embedded/step-by-step-guide/index.md) and the progressive [Meet tutorials](https://openvidu.io/3.8/meet/embedded/tutorials/index.md), from direct links to webhooks.
- [Angular Components tutorials](https://openvidu.io/3.8/docs/tutorials/angular-components/index.md), one per customizable piece.
- [Build your app](https://openvidu.io/3.8/docs/build-your-app/index.md) and the [Getting started](https://openvidu.io/3.8/docs/getting-started/index.md) page for the SDK path.
- [Deployment types](https://openvidu.io/3.8/docs/self-hosting/deployment-types/index.md) when you are ready to leave `localhost`: Single Node, Elastic or High Availability, on your servers or any major cloud.
