...
OpenVidu Meet embedded in an AI-generated CRM — a real meeting with the participants panel open, inside the CRM
...
We recently ran this experiment: build a small CRM — clients, support issues, scheduled meetings — entirely with an AI agent, then ask the agent to integrate [OpenVidu Meet](/meet/embedded/intro/) so that meetings happen *inside* the app instead of in an external tool. Every prompt we typed is preserved in the repository, so this post is a faithful, reproducible walkthrough of the whole journey — including the parts where reality pushed back.
...
The clients page of the generated CRM

The meetings calendar of the generated CRM
...
Architecture: the CRM server provisions rooms and members through the OpenVidu Meet REST API; the browser embeds the meeting with the webcomponent

An issue with its planned and past meetings
...
The missing members API shipped days later in [OpenVidu Meet 3.8.0](https://openvidu.io/3.8/blog/2026/07/09/release-380/index.md), turning the "share a link" model into a complete access control system. One more prompt brought the CRM up to date:
...
The client page showing meeting room access

The fine-grained permissions editor
