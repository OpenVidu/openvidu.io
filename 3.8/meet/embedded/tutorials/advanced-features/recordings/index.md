...
[Source code](https://github.com/OpenVidu/openvidu-meet-tutorials/tree/3.8.0/advanced-features/meet-recordings)

This tutorial extends the [advanced OpenVidu Meet WebComponent tutorial](https://openvidu.io/3.8/meet/embedded/tutorials/embedding-options/webcomponent-advanced/index.md) to add **recording management capabilities**. It demonstrates how to list, view, and delete recordings from your OpenVidu Meet meetings.
...
This tutorial builds upon the [advanced OpenVidu Meet WebComponent tutorial](https://openvidu.io/3.8/meet/embedded/tutorials/embedding-options/webcomponent-advanced/index.md), adding recording management functionality. We'll focus on the new features and modifications related to recordings.
...
[index.js](https://github.com/OpenVidu/openvidu-meet-tutorials/blob/3.8.0/advanced-features/meet-recordings/src/index.js#L74-L92)
...
1. If a room name is provided, it appends the `roomName` parameter to the recordings path to filter recordings by that room.
...
1. Extracts an optional room name from the query parameters for filtering. If a room name is provided, it appends the `roomName` parameter to the recordings path to filter recordings by that room.
...
[index.js](https://github.com/OpenVidu/openvidu-meet-tutorials/blob/3.8.0/advanced-features/meet-recordings/src/index.js#L94-L105)
...
[index.js](https://github.com/OpenVidu/openvidu-meet-tutorials/blob/3.8.0/advanced-features/meet-recordings/src/index.js#L107-L118)
...
[app.js](https://github.com/OpenVidu/openvidu-meet-tutorials/blob/3.8.0/advanced-features/meet-recordings/public/js/app.js#L1-L2)
...
[app.js](https://github.com/OpenVidu/openvidu-meet-tutorials/blob/3.8.0/advanced-features/meet-recordings/public/js/app.js#L51-L101)
...
[app.js](https://github.com/OpenVidu/openvidu-meet-tutorials/blob/3.8.0/advanced-features/meet-recordings/public/js/app.js#L216-L228)
...
[app.js](https://github.com/OpenVidu/openvidu-meet-tutorials/blob/3.8.0/advanced-features/meet-recordings/public/js/app.js#L238-L265)
...
1. Make a `GET` request to the `/recordings` endpoint to fetch the list of recordings. The backend already returns only completed recordings, so no client-side filtering is needed.
1. For each recording in the returned list, add it to the `recordings` map indexed by recording ID.
...
1. Makes a `GET` request to the `/recordings` endpoint to fetch the list of recordings, including the room filter parameter if specified. The backend endpoint already filters to completed recordings only, so no client-side status filtering is required.
1. For each recording in the returned list, it adds the recording to the `recordings` map. This map is used to store the recordings indexed by their recording IDs to make it easier to access them later.
...
[app.js](https://github.com/OpenVidu/openvidu-meet-tutorials/blob/3.8.0/advanced-features/meet-recordings/public/js/app.js#L267-L287)
...
1. For each recording, get the HTML template for the recording list item, then join all the items into a single HTML string assigned to the list element.
...
1. Maps every recording to its HTML template using the `getRecordingListItemTemplate()` function and joins them into a single string that is assigned to the list element's inner HTML.

The `getRecordingListItemTemplate()` function generates the HTML template for each recording list item:

[app.js](https://github.com/OpenVidu/openvidu-meet-tutorials/blob/3.8.0/advanced-features/meet-recordings/public/js/app.js#L289-L315)
...
[app.js](https://github.com/OpenVidu/openvidu-meet-tutorials/blob/3.8.0/advanced-features/meet-recordings/public/js/app.js#L317-L353)
...
1. Add an event listener for the `closed` event, which is triggered when the recording playback view is closed. It hides the playback screen and shows the recordings list screen again.
...
[app.js](https://github.com/OpenVidu/openvidu-meet-tutorials/blob/3.8.0/advanced-features/meet-recordings/public/js/app.js#L355-L365)
