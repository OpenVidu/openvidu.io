...
This local deployment is the ideal choice to develop the [embedding of OpenVidu Meet into your own application](https://openvidu.io/3.8/meet/embedded/intro/index.md).

Warning

Remember that this deployment is designed for **demo** and **development** purposes. For production follow the [Basic deployment](https://openvidu.io/3.8/meet/deployment/basic/index.md) or [Advanced deployments](https://openvidu.io/3.8/meet/deployment/advanced/index.md).
...
1. Copy and paste the following command into the terminal:
   ...
   If you want to deploy a specific version, replace `latest` with the desired version tag. E.g., to deploy version `3.8.0` use:

   ```text
   docker compose -p openvidu-meet -f oci://openvidu/local-meet:3.8.0 up -y openvidu-meet-init
   ```

   Command failing on Windows with a `dockerHttpProxy` error
...
You can use the [REST API](https://openvidu.io/3.8/meet/embedded/reference/rest-api/index.md) to [embed OpenVidu Meet](https://openvidu.io/3.8/meet/embedded/intro/index.md) using:

- API Key: `meet-api-key`

You can change them later from the [OpenVidu Meet app](https://openvidu.io/3.8/meet/features/users/management/#changing-credentials).
...
1. Access OpenVidu Meet with a different URL:
...
If you want to modify some configurations or have more control over the local deployment, you can deploy the [OpenVidu Platform Local deployment](https://openvidu.io/3.8/docs/self-hosting/local/index.md) which by default includes **OpenVidu Meet** as one of its services.
