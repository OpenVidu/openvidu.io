...
E2EE can be enabled or disabled on a per-room basis when [creating](https://openvidu.io/3.8/meet/features/rooms/management/#create-rooms) or [editing a room](https://openvidu.io/3.8/meet/features/rooms/management/#edit-rooms), from the **Room Features** step of the configuration wizard.

Room creation wizard with the end-to-end encryption option enabled

E2EE Limitations

- **Recording is not available** for encrypted rooms: the server cannot decrypt the content, so it cannot produce a [recording](https://openvidu.io/3.8/meet/features/recordings/overview/index.md). To record, disable E2EE for the room.
...
When a member accesses a E2E-encrypted room, the [Lobby view](https://openvidu.io/3.8/meet/features/meetings/lifecycle/#lobby-view) shows an **"end-to-end encrypted"** badge and a required passphrase field. With the correct passphrase, the meeting works like any other.

Lobby view with the end-to-end encrypted badge and passphrase field
...
Meeting seen by a participant with a wrong passphrase: remote videos are unreadable
