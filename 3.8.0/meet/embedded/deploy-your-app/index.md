...
Once your application built with [OpenVidu Meet Embedded](https://openvidu.io/3.8.0/meet/embedded/intro/index.md) is ready for production, you need to decide where to deploy it. Before proceeding, make sure you have a running OpenVidu Meet deployment. If you don't have one yet, check the [Deployment](https://openvidu.io/3.8.0/meet/deployment/overview/index.md) section to set one up.
...
All examples in this guide use the default OpenVidu Meet base path `/meet`. If you have changed it using the `MEET_BASE_PATH` parameter in `openvidu.env`, replace `/meet` with your configured path in all URLs mentioned below. See the [Customize OpenVidu Meet base path](https://openvidu.io/3.8.0/docs/self-hosting/how-to-guides/customize-meet-base-path/index.md) how-to guide for more details.
...
See the [REST API reference](https://openvidu.io/3.8.0/meet/embedded/reference/rest-api/index.md) for the full list of available endpoints.
...
Use this API key in your application to authenticate REST API requests via the `X-API-KEY` header. See the [REST API authentication](https://openvidu.io/3.8.0/meet/embedded/reference/rest-api/#authentication) section for more details.

### 4. Configure the Web Component URL (if applicable)

If your application uses the [OpenVidu Meet Web Component](https://openvidu.io/3.8.0/meet/embedded/reference/webcomponent/index.md), make sure the script tag points to your OpenVidu deployment's public domain:
...
If your application uses [OpenVidu Meet webhooks](https://openvidu.io/3.8.0/meet/embedded/reference/webhooks/index.md), you need to configure the webhook URL to point to your application locally:

1. Navigate to `https://your-domain/meet`.

1. Go to the **"Embedded"** page.

1. Enable webhooks and set the webhook URL to:
...
1. You can use the **"Test webhook"** button to verify that your application receives webhook events correctly.
...
All webhook events are signed with your API key using HMAC SHA256. Always [validate webhook signatures](https://openvidu.io/3.8.0/meet/embedded/reference/webhooks/#validate-events) in your application to ensure authenticity and prevent tampering.
...
See the [REST API authentication](https://openvidu.io/3.8.0/meet/embedded/reference/rest-api/#authentication) section for more details.

### 3. Configure the Web Component URL (if applicable)

If your application uses the [OpenVidu Meet Web Component](https://openvidu.io/3.8.0/meet/embedded/reference/webcomponent/index.md), make sure the script tag points to your OpenVidu deployment:
...
If your application uses [OpenVidu Meet webhooks](https://openvidu.io/3.8.0/meet/embedded/reference/webhooks/index.md), configure the webhook URL to point to your application's **publicly accessible** webhook endpoint:

1. Navigate to your OpenVidu Meet app at `https://your-openvidu-domain/meet`.

1. Go to the **"Embedded"** page.

1. Enable webhooks and set the webhook URL to your application's public endpoint:
...
See the [Configuration Reference](https://openvidu.io/3.8.0/docs/self-hosting/configuration/reference/#meetenv) for all available `meet.env` parameters and the [Changing Configuration](https://openvidu.io/3.8.0/docs/self-hosting/configuration/changing-config/index.md) section for more details on how to modify configuration files.
