...
- **Over the REST API:** the links come back on the room object from [`GET /rooms/{roomId}`](https://openvidu.io/3.8.0/meet/embedded/reference/api.html#/operations/getRoom), at `access.anonymous.moderator.url` and `access.anonymous.speaker.url`; enable or disable each role with [`PUT /rooms/{roomId}/access`](https://openvidu.io/3.8.0/meet/embedded/reference/api.html#/operations/updateRoomAccess).

See [Room Access](https://openvidu.io/3.8.0/meet/features/rooms/access/index.md) for the full picture.
...
- **Over the REST API:** create them with [`POST /rooms/{roomId}/members`](https://openvidu.io/3.8.0/meet/embedded/reference/api.html#/operations/addRoomMember) — the response carries the personal link in the member's `accessUrl` (also retrievable via [`GET /rooms/{roomId}/members/{memberId}`](https://openvidu.io/3.8.0/meet/embedded/reference/api.html#/operations/getRoomMember)).

See [Room Members](https://openvidu.io/3.8.0/meet/features/room-members/overview/index.md).
...
- **Over the REST API:** create accounts with [`POST /users`](https://openvidu.io/3.8.0/meet/embedded/reference/api.html#/operations/createUser), add a user as a member with [`POST /rooms/{roomId}/members`](https://openvidu.io/3.8.0/meet/embedded/reference/api.html#/operations/addRoomMember), and read the shared link at `access.user.url` on the room object from [`GET /rooms/{roomId}`](https://openvidu.io/3.8.0/meet/embedded/reference/api.html#/operations/getRoom).

See [Users](https://openvidu.io/3.8.0/meet/features/users/overview/index.md).
...
Notice that all three models answer "how do you get *in*." A separate question is "what can you *do* once you're in" — and that layer is **orthogonal**. You can attach the same fine-grained permission set to an anonymous guest, an identified guest, or a user.
...
👉 **See it in action — [spin up a permissioned room with OpenVidu Meet Embedded](https://openvidu.io/3.8.0/meet/embedded/intro/index.md).**

To go deeper on the concepts covered here:

- [Users](https://openvidu.io/3.8.0/meet/features/users/overview/index.md) — accounts, roles and org-wide access rules.
- [Room Members](https://openvidu.io/3.8.0/meet/features/room-members/overview/index.md) — identified guests vs users.
- [Room Access](https://openvidu.io/3.8.0/meet/features/rooms/access/index.md) — the three access-link types.
- Access tutorials for each model: [anonymous access](https://openvidu.io/3.8.0/meet/embedded/tutorials/access/anonymous-access/index.md), [identified guests](https://openvidu.io/3.8.0/meet/embedded/tutorials/access/identified-guests/index.md), and [users](https://openvidu.io/3.8.0/meet/embedded/tutorials/access/users/index.md).

And if you need total control over the media pipeline instead, [OpenVidu Platform](https://openvidu.io/3.8.0/docs/index.md) gives you the low-level SDKs to build your own access layer from the ground up.
