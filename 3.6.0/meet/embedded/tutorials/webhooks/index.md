...
[Source code](https://github.com/OpenVidu/openvidu-meet-tutorials/tree/3.6.0/meet-webhooks)

This tutorial extends the [recordings tutorial](https://openvidu.io/3.6.0/meet/embedded/tutorials/recordings/index.md) to add **real-time updates** through webhooks and Server-Sent Events (SSE). It demonstrates how to receive and process OpenVidu Meet webhooks to provide live status updates for rooms and recordings.
...
```bash
git clone https://github.com/OpenVidu/openvidu-meet-tutorials.git -b 3.6.0
```
...
This tutorial builds upon the [recordings tutorial](https://openvidu.io/3.6.0/meet/embedded/tutorials/recordings/index.md), adding real-time functionality through webhooks and Server-Sent Events. We'll focus on the new webhook handling capabilities and live update features.
...
You can find examples of how to verify OpenVidu Meet webhooks in other programming languages in the [Validate events](https://openvidu.io/3.6.0/meet/embedded/reference/webhooks/#validate-events) section of the Webhooks reference.
...
1. **Configure OpenVidu Meet for network access**: Start OpenVidu Meet by following the instructions in the [Accessing OpenVidu Meet from other computers or phones](https://openvidu.io/3.6.0/meet/deployment/local/#accessing-openvidu-meet-from-other-computers-or-phones) section.
...
If you have a production deployment of OpenVidu Meet (installed in a server following [deployment steps](https://openvidu.io/3.6.0/meet/deployment/basic/index.md) ), you can connect this tutorial to it by following these steps:
...
1. **Update the API key**: Ensure the `OV_MEET_API_KEY` environment variable in the `.env` file matches the API key configured in your production deployment. See [Generate an API Key](https://openvidu.io/3.6.0/meet/embedded/reference/rest-api/#generate-an-api-key) section to learn how to obtain it.
...
1. **Configure webhooks in OpenVidu Meet**: Set up webhooks in your OpenVidu Meet production deployment to point to this tutorial. Follow the instructions in the [Webhooks configuration](https://openvidu.io/3.6.0/meet/embedded/reference/webhooks/#configuration) section to learn how to configure a webhook URL. Use the public URL of this tutorial followed by `/webhook` (e.g., `https://your-tutorial-domain.com:XXXX/webhook`).
