...
The key element of any WebRTC server solution is the ability to exchange media between participants of a room, in the so-called [WebRTC SFU](https://openvidu.io/3.6.0/docs/comparing-openvidu/#openvidu-vs-sfus). LiveKit implements its own SFU, and that's where OpenVidu makes a different choice by using [mediasoup](https://mediasoup.org/) .
...
In terms of the signaling protocol, API and SDKs, OpenVidu maintains the original LiveKit implementation. LiveKit's API is very well designed, with a simple but powerful [set of concepts](https://openvidu.io/3.6.0/docs/getting-started/#basic-concepts), and the amount of SDKs available is very large.
