...
This tutorial extends the [External Members tutorial](https://openvidu.io/3.7.0/meet/embedded/tutorials/external-members/index.md) to show how to create **OpenVidu Meet users** with the Users API and grant them access to a room as **registered members**.
...
The application uses the [OpenVidu Meet API](https://openvidu.io/3.7.0/meet/embedded/reference/rest-api/index.md) to manage users, rooms and room members, and the [OpenVidu Meet WebComponent](https://openvidu.io/3.7.0/meet/embedded/reference/webcomponent/index.md) to embed the meeting.

Anonymous access vs. room members

OpenVidu Meet rooms can be accessed either through **anonymous access links** (used by the other tutorials) or by adding **room members** with personalized access and permissions. Registered members are one of the two kinds of room members (the other being [external members](https://openvidu.io/3.7.0/meet/embedded/tutorials/external-members/index.md)). See [Room Access](https://openvidu.io/3.7.0/meet/features/rooms/access/#room-members) for the full picture.
...
This tutorial builds upon the [External Members tutorial](https://openvidu.io/3.7.0/meet/embedded/tutorials/external-members/index.md): the room and member management, the application views and the shared helper functions are the same. Here we focus only on what is new — the **Users API** — and on the **small changes** needed to manage **registered** members instead of external ones.
...
The room endpoints (`POST /rooms`, `GET /rooms`, `DELETE /rooms/:roomId`) and the `DELETE /rooms/:roomId/members/:memberId` endpoint are identical to the [External Members tutorial](https://openvidu.io/3.7.0/meet/embedded/tutorials/external-members/index.md).
