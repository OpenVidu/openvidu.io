...
If none of these scenarios apply to you and you prefer to use the default internal Caddy server, please refer to the [official installation guides](https://openvidu.io/3.5/docs/self-hosting/deployment-types/index.md).
...
> **Note**
>
> The Single Node deployment with an external proxy is based on the same instructions as the [Single Node COMMUNITY Deployment](https://openvidu.io/3.5/docs/self-hosting/single-node/on-premises/install/index.md) and the [Single Node PRO Deployment](https://openvidu.io/3.5/docs/self-hosting/single-node-pro/on-premises/install/index.md), but with some modifications to the installation command and port rules. We recommend you to read the installation guides before proceeding with this guide to have a better understanding of the deployment.
...
You can follow the same rule ports of the [Single Node Deployment](https://openvidu.io/3.5/docs/self-hosting/single-node/on-premises/install/#port-rules) but some ports are used by the proxy server and others are not needed. The inbound rules for the OpenVidu proxy would be as follows:
...
> **Note**
>
> In case you want to deploy a specific version, just replace `latest` with the desired version. For example: `3.5.0`.
...
We will use [Nginx](https://www.nginx.com/) as the proxy server, but the configuration can be adapted to other proxy servers. The configuration for the proxy server is as follows:
...
1. Configure a rule to proxy all Layer 4 TLS incoming proxy traffic in port 443 to the OpenVidu Master Node in port 7880 for domain `openvidu.example.io`. This is for the HTTP traffic of the OpenVidu API and other services.
1. Configure a rule to proxy all Layer 4 TLS incoming proxy traffic in port 443 to the OpenVidu Master Node in port 5349 for domain `turn.example.io`. This is for the TURN service.
1. Configure a rule to proxy all Layer 4 TLS incoming proxy traffic in port 1935 to the OpenVidu Master Node in port 1945 for RTMP traffic.
...
1. Configure a rule to proxy all Layer 7 HTTPS incoming proxy traffic in port 443 to the OpenVidu Master Node in port 7880.
1. Configure a rule to proxy all Layer 4 TLS incoming proxy traffic in port 1935 to the OpenVidu Master Node in port 1945.
...
1. Configure a rule to redirect HTTP traffic to HTTPS in port 80.
1. Configure a rule to proxy all Layer 4 TLS incoming proxy traffic in port 443 to the OpenVidu Master Node in port 7880.
1. Configure a rule to proxy all Layer 4 TLS incoming proxy traffic in port 1935 to the OpenVidu Master Node in port 1945.
...
> **Note**
>
> The Elastic deployment with an external proxy is based on the same instructions as the [Elastic Deployment](https://openvidu.io/3.5/docs/self-hosting/elastic/on-premises/install/index.md), but with some modifications to the installation command and port rules. We recommend you to read the [Elastic Deployment](https://openvidu.io/3.5/docs/self-hosting/elastic/on-premises/install/index.md) guide before proceeding with this guide to have a better understanding of the deployment.
...
- An additional machine for the proxy server is recommended. Alternatively, you can use the same machine as the Master Node, but be aware that the proxy server will consume resources. Note that [some ports will be used by OpenVidu](https://openvidu.io/3.5/docs/self-hosting/elastic/on-premises/install/#port-rules-master-node), except for the ports utilized by the proxy server (TCP 80, 443, and 1935).
...
You can follow the same rule ports of the Elastic Deployment for the [Master Node](https://openvidu.io/3.5/docs/self-hosting/elastic/on-premises/install/#port-rules-master-node) and for the [Media Nodes](https://openvidu.io/3.5/docs/self-hosting/elastic/on-premises/install/#port-rules-media-nodes) but some ports are used by the proxy server and others are not needed. The inbound rules for the OpenVidu proxy would be as follows:
...
The High Availability deployment already has a way to configure an external proxy (described as a Network Load Balancer), which is explained [in this section](https://openvidu.io/3.5/docs/self-hosting/ha/on-premises/install-nlb/index.md).
