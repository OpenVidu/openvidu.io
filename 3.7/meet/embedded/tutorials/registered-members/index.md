...
[Source code](https://github.com/OpenVidu/openvidu-meet-tutorials/tree/3.7.0/meet-registered-members)

This tutorial extends the [External Members tutorial](https://openvidu.io/3.7/meet/embedded/tutorials/external-members/index.md) to show how to create **OpenVidu Meet users** with the Users API and grant them access to a room as **registered members**.
...
The application uses the [OpenVidu Meet API](https://openvidu.io/3.7/meet/embedded/reference/rest-api/index.md) to manage users, rooms and room members, and the [OpenVidu Meet WebComponent](https://openvidu.io/3.7/meet/embedded/reference/webcomponent/index.md) to embed the meeting.

Anonymous access vs. room members

OpenVidu Meet rooms can be accessed either through **anonymous access links** (used by the other tutorials) or by adding **room members** with personalized access and permissions. Registered members are one of the two kinds of room members (the other being [external members](https://openvidu.io/3.7/meet/embedded/tutorials/external-members/index.md)). See [Room Access](https://openvidu.io/3.7/meet/features/rooms/access/#room-members) for the full picture.
...
For a detailed guide on how to run OpenVidu Meet locally, visit [Try OpenVidu Meet locally](https://openvidu.io/3.7/meet/deployment/local/index.md) .
...
This tutorial builds upon the [External Members tutorial](https://openvidu.io/3.7/meet/embedded/tutorials/external-members/index.md): the room and member management, the application views and the shared helper functions are the same. Here we focus only on what is new — the **Users API** — and on the **small changes** needed to manage **registered** members instead of external ones.
...
The room endpoints (`POST /rooms`, `GET /rooms`, `DELETE /rooms/:roomId`) and the `DELETE /rooms/:roomId/members/:memberId` endpoint are identical to the [External Members tutorial](https://openvidu.io/3.7/meet/embedded/tutorials/external-members/index.md).
...
1. The user is created with the `room_member` role. Among the available roles (`admin`, `user` and `room_member`), `room_member` is the most restricted: it can only access the rooms where it has been explicitly added as a member, and cannot create or manage rooms.
...
1. Providing `userId` (and **not** `name`) is what tells the API to create a member of type `registered`. The member is linked to the user account, identified through authentication, and shares the same authenticated room access URL with the rest of the registered members.

Info

As in the External Members tutorial, you can fine-tune the member's permissions beyond the base role by including a `customPermissions` object in the request. See the [`addRoomMember` operation](https://openvidu.io/3.7/meet/embedded/reference/api.html#/operations/addRoomMember) in the REST API reference for the full list of permissions.
...
1. Add the new user to the local `users` map and re-render the list.
...
1. Make a `POST` request to the `/rooms/:roomId/members` endpoint to add the member to the current room (`currentRoom` is the room whose members are being managed).
1. Add the returned member to the local `members` map and re-render the list.
...
1. Inject the OpenVidu Meet WebComponent with the `room-url` attribute set to that URL.
...
1. **Configure OpenVidu Meet for network access**: Start OpenVidu Meet by following the instructions in the [Accessing OpenVidu Meet from other computers or phones](https://openvidu.io/3.7/meet/deployment/local/#accessing-openvidu-meet-from-other-computers-or-phones) section.
...
If you have a production deployment of OpenVidu Meet (installed in a server following [deployment steps](https://openvidu.io/3.7/meet/deployment/basic/index.md) ), you can connect this tutorial to it by following these steps:
...
1. **Update the API key**: Ensure the `OV_MEET_API_KEY` environment variable in the `.env` file matches the API key configured in your production deployment. See [Generate an API Key](https://openvidu.io/3.7/meet/embedded/reference/rest-api/#generate-an-api-key) section to learn how to obtain it.
