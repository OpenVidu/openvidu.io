...
1. [Stop](#stop-openvidu-single-node-pro) the instance if it is not already stopped. Wait for it to reach the **Stopped** state.
...
Regarding the administration of your deployment, you can follow the instructions in section [On Premises Single Node Administration](https://openvidu.io/3.7/docs/self-hosting/single-node-pro/on-premises/admin/index.md).
...
1. Go to the [OCI Compute Instances](https://cloud.oracle.com/compute/instances) and click on [*Stop*](#stop-openvidu-single-node-pro) → [*Start*](#start-openvidu-single-node-pro) to apply the changes to the OpenVidu Single Node PRO deployment.
