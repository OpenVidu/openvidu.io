...
Internally, the Azure Elastic deployment mirrors the On Premises Elastic deployment, allowing you to follow the same administration and configuration guidelines of the [On Premises Elastic](https://openvidu.io/3.7/docs/self-hosting/elastic/on-premises/admin/index.md) documentation. However, there are specific considerations unique to the Azure environment that are worth keeping in mind:
...
1. Then click into the Virtual Machine Scale Set resource called `<STACK_NAME>-mediaNodeScaleSet` and click *"Availability + scale"* on the left panel, then click the *"Scaling"* option.
1. On this tab, scroll to the bottom and modify the *"Instance Limits"* to 0.
...
1. Navigate to the [Azure Portal Dashboard](https://portal.azure.com/#home) and go to the Resource Group where you deployed OpenVidu Elastic.
1. In the resource group click on the resource called *"stackName-VM-MasterNode"*, then click *"Start"* to start the Master Node.
...
1. On this tab, scroll to the bottom and modify the *"Instance Limits"* to the value of fixed number of Media Nodes you want. In this case it is set to 2.
1. Click *"Save"* and wait for it to complete. You can check the progress in the *"Instances"* tab.

## Administration and configuration

Regarding the administration of your deployment, you can follow the instructions in section [On Premises Elastic Administration](https://openvidu.io/3.7/docs/self-hosting/elastic/on-premises/admin/index.md).
