# OpenVidu Single Node COMMUNITY installation: DigitalOcean
...
This page explains how to create a Droplet (VM) in DigitalOcean, configure networking, and prepare it for OpenVidu Single Node On-Premises. Installing, administering, and upgrading OpenVidu Single Node itself is covered in the On-Premises documentation.
...
1. Search for **Droplets**, click it, and then click *"Create Droplet"*.
1. Choose a region and then change the image to Ubuntu *"24.04 (LTS) x64"* if it is not selected yet.
...
1. Scroll down to Authentication Method and choose the one you prefer. This will be used to connect to the instance via terminal. If you want to use an SSH key, follow the instructions shown when you click New SSH Key.
1. Review the configuration and click *"Create Droplet"*, you can change the hostname of the droplet if you want (for example, `openvidu-singlenode`).
...
The [minimum inbound ports to allow](https://openvidu.io/3.7/docs/self-hosting/single-node/on-premises/install/#port-rules) must be included in the Firewall rules.

1. Click the droplet, then go to *"Networking"*, scroll down and click on *"Edit"* in **Firewall** section.

1. Now click on *"Create Firewall"* and in **Inbound Rules** add the following rules.

   > **Warning**
   >
   > It is important that you make sure the protocol is the one that is shown in the image.

1. Name the firewall, then scroll to the bottom and search for your Droplet by name. Select it to apply the firewall rules to it.
...
1. Follow the [On-Premises install instructions](https://openvidu.io/3.7/docs/self-hosting/single-node/on-premises/install/#guided-installation) to install OpenVidu on the instance.

______________________________________________________________________

### 4. Administration and upgrade

- For administration of this OpenVidu Single Node deployment, see the [On-Premises administration section](https://openvidu.io/3.7/docs/self-hosting/single-node/on-premises/admin/index.md).
- To upgrade OpenVidu, see the [On-Premises upgrade section](https://openvidu.io/3.7/docs/self-hosting/single-node/on-premises/upgrade/index.md).

## **Terraform**

This section contains instructions for deploying a production-ready OpenVidu Single Node COMMUNITY deployment on DigitalOcean. The deployed services are the same as in the [On Premises Single Node installation](https://openvidu.io/3.7/docs/self-hosting/single-node/on-premises/install/index.md), but the process is automated through the Terraform CLI. Additionally, DigitalOcean Spaces (S3-compatible storage) is used to store recordings and other persistent data.
...
OpenVidu Single Node DigitalOcean Architecture
...
1. Copy **terraform.tfvars.example** to **terraform.tfvars**, update the required parameters with your values, and optionally adjust defaults.
   ...
   > | `domainName`               | `(none)`        | Domain name for the OpenVidu Deployment. Not mandatory; if not provided, a sslip.io domain will be used instead.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
   > | `ownPublicCertificate`     | `(none)`        | If certificate type is 'owncert', this parameter will be used to specify your public certificate in base64.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
   > | `ownPrivateCertificate`    | `(none)`        | If certificate type is 'owncert', this parameter will be used to specify your private certificate in base64.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
You may need your DigitalOcean credentials to configure your OpenVidu application. You can check these secrets following these steps ([View OpenVidu credentials in the Web](#view-openvidu-credentials-in-the-web)) or ([View OpenVidu credentials in the instance](#view-openvidu-credentials-in-the-instance)).
...
1. If everything seems fine, check the [status](https://openvidu.io/3.7/docs/self-hosting/single-node/on-premises/admin/#checking-the-status-of-services) and the [logs](https://openvidu.io/3.7/docs/self-hosting/single-node/on-premises/admin/#checking-logs) of the installed OpenVidu services.

### Configuration and administration

When your **OPENVIDU_URL** is reachable, it means that everything has gone well. Now you can check the [Administration](https://openvidu.io/3.7/docs/self-hosting/single-node/digitalocean/admin/index.md) section to learn how to manage your deployment.
