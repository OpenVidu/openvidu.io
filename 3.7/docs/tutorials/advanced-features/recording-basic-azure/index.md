...
[Source code](https://github.com/OpenVidu/openvidu-livekit-tutorials/tree/3.7.0/advanced-features/openvidu-recording-basic-node-azure)

This tutorial is a simple video-call application, built upon [Node.js server](https://openvidu.io/3.7/docs/tutorials/application-server/node/index.md) and [JavaScript client](https://openvidu.io/3.7/docs/tutorials/application-client/javascript/index.md) tutorials, and extends them by adding recording capabilities:
...
1. Deploy a Storage Account in Azure and add a container with the name that you want.

1. Change in the egress.yaml the S3 configuration for the Azure configuration you will find commented and fill the configuration with your credentials
...
1. Deploy OpenVidu Single Node in Azure following these instructions [to deploy in Azure](https://openvidu.io/3.7/docs/self-hosting/single-node/azure/install/index.md).

   > **CPUs to be able to record**
   >
   > Make sure you deploy with at least 4 CPUs in the Virtual Machine of Azure.

1. Point the tutorial to your Azure deployment:

   - Modify file [`.env`](https://github.com/OpenVidu/openvidu-livekit-tutorials/blob/4e90828d801208945fc33aede4cd994abcacdc91/advanced-features/openvidu-recording-basic-node-azure/.env) to update the LiveKit and Azure configuration to the values of your Azure deployment. You can get the values of `LIVEKIT_URL`, `LIVEKIT_API_KEY` and `LIVEKIT_API_SECRET` from the [Check deployment outputs in Azure Key Vault](https://openvidu.io/3.7/docs/self-hosting/single-node/azure/install/#check-deployment-outputs-in-azure-key-vault) section. You can get the values of `AZURE_ACCOUNT_NAME`, `AZURE_ACCOUNT_KEY` and `AZURE_CONTAINER_NAME` from the `openvidu.env` file of your deployment (see [Azure SSH Outputs](https://openvidu.io/3.7/docs/self-hosting/single-node/azure/install/#check-deployment-outputs-in-the-instance)).
...
>
> Access your application client through [`https://xxx-yyy-zzz-www.openvidu-local.dev:6443`](https://xxx-yyy-zzz-www.openvidu-local.dev:6443), where `xxx-yyy-zzz-www` part of the domain is your LAN private IP address with dashes (-) instead of dots (.). For more information, see section [Accessing your local deployment from other devices on your network](https://openvidu.io/3.7/docs/self-hosting/local/#accessing-your-local-deployment-from-other-devices-on-your-network).
...
The server application extends the [Node.js server tutorial](https://openvidu.io/3.7/docs/tutorials/application-server/node/index.md) by adding the following REST API endpoints:
...
[index.js](https://github.com/OpenVidu/openvidu-livekit-tutorials/blob/3.7.0/advanced-features/openvidu-recording-basic-node-azure/src/index.js#L15-L26)
...
It also initializes the `EgressClient`, which will help interacting with [Egress API](https://docs.livekit.io/reference/other/egress/api/) to manage recordings, and the `AzureBlobStorageService`, which will help interacting with the Blob Container:

[index.js](https://github.com/OpenVidu/openvidu-livekit-tutorials/blob/3.7.0/advanced-features/openvidu-recording-basic-node-azure/src/index.js#L33-L38)
...
[index.js](https://github.com/OpenVidu/openvidu-livekit-tutorials/blob/3.7.0/advanced-features/openvidu-recording-basic-node-azure/src/index.js#L45-L62)
...
[index.js](https://github.com/OpenVidu/openvidu-livekit-tutorials/blob/3.7.0/advanced-features/openvidu-recording-basic-node-azure/src/index.js#L79-L117)
...
1. Define the file path where the recording will be stored. The `{room_name}`, `{time}` and `{room_id}` templates will be replaced by the actual room name, timestamp and room ID, respectively. Check out all available [filename templates](https://docs.livekit.io/transport/media/ingress-egress/egress/outputs/#filename-templating) .
...
1. Check if there is already an active recording for the room. If there is, it returns a `409` error to prevent starting a new recording. To accomplish this, we use the `getActiveRecordingByRoom` function, which lists all active egresses for a specified room by calling the `listEgress` method of the `EgressClient` with the `roomName` and `active` parameters, and then returns the egress ID of the first active egress found:
   ...
   [index.js](https://github.com/OpenVidu/openvidu-livekit-tutorials/blob/3.7.0/advanced-features/openvidu-recording-basic-node-azure/src/index.js#L231-L240)
...
[index.js](https://github.com/OpenVidu/openvidu-livekit-tutorials/blob/3.7.0/advanced-features/openvidu-recording-basic-node-azure/src/index.js#L120-L148)
...
[index.js](https://github.com/OpenVidu/openvidu-livekit-tutorials/blob/3.7.0/advanced-features/openvidu-recording-basic-node/src/index.js#L152-L173)
...
1. List all Egress video files in the Azure Container.
1. Map all of the recording names from the Azure response.
...
1. Lists all Egress video files in the Azure Container. To accomplish this, we use the `listObjects` method of the `AzureBlobService` with the `RECORDINGS_PATH` parameter.
1. Extracts the recording names from the Azure response.
...
[index.js](https://github.com/OpenVidu/openvidu-livekit-tutorials/blob/3.7.0/advanced-features/openvidu-recording-basic-node-azure/src/index.js#L173-L203)
...
1. Get the recording file from the Azure Container.
...
1. Checks if the recording exists in the Azure Container by calling the `exists` method of the `AzureBlobService` with the `recordingName` as a parameter. If the recording does not exist, it returns a `404` error.

1. Gets the requested range of the recording file by calling the `getRecordingStream` function:

   [index.js](https://github.com/OpenVidu/openvidu-livekit-tutorials/blob/3.7.0/advanced-features/openvidu-recording-basic-node-azure/src/index.js#L242-L255)
   ...
   1. Get the recording file from the Azure Container with the requested range.
   ...
   1. Gets the recording file from the Azure Container with the requested range by calling the `getObject` method of the `AzureBlobService` with the `recordingName` and `range` as parameters.
...
[index.js](https://github.com/OpenVidu/openvidu-livekit-tutorials/blob/3.7.0/advanced-features/openvidu-recording-basic-node-azure/src/index.js#L206-L222)
...
1. Check if the recording exists in the Azure Container.
1. Delete the recording file from the Azure Container.

This endpoint does the following:

1. Extracts the `recordingName` parameter from the request.
1. Checks if the recording exists in the Azure Container by calling the `exists` method of the `AzureBlobService` with the `recordingName` as a parameter. If the recording does not exist, it returns a `404` error.
1. Deletes the recording file from the Azure Container by calling the `deleteObject` method of the `AzureBlobService` with the object's `recordingName` as a parameter.
...
[azure.blobstorage.service.js](https://github.com/OpenVidu/openvidu-livekit-tutorials/blob/3.7.0/advanced-features/openvidu-recording-basic-node-azure/src/azure.blobstorage.service.js)
...
1. The access key of Azure Blob Storage.
1. The name of Azure Container.
1. The URL of Azure endpoint.
1. Initialize the `Clients` with the provided configuration.
1. Check if an object exists in the Azure Container.
1. Retrieve the metadata of an object in the Azure Container.
1. Retrieve the size of an object in the Azure Container.
1. Retrieve a specified range of bytes from an object in the Azure Container.
1. List objects in the Azure Container that match a regex pattern.
1. Delete an object from the Azure Container.
...
>
> To overcome these limitations, you can follow the steps described in the [advanced recording tutorial](https://openvidu.io/3.7/docs/tutorials/advanced-features/recording-advanced-azure/index.md), where we implement a custom notification system. This system informs participants about the recording status by listening to webhook events and updating room metadata.

[app.js](https://github.com/OpenVidu/openvidu-livekit-tutorials/blob/3.7.0/advanced-features/openvidu-recording-basic-node-azure/public/app.js#L20-L87)
...
[app.js](https://github.com/OpenVidu/openvidu-livekit-tutorials/blob/3.7.0/advanced-features/openvidu-recording-basic-node-azure/public/app.js#L337-L370)
...
>
> In the [advanced recording tutorial](https://openvidu.io/3.7/docs/tutorials/advanced-features/recording-advanced-azure/index.md), we show how to implement a custom notification system that alerts all participants of a recording's deletion by sending data messages.
...
[app.js](https://github.com/OpenVidu/openvidu-livekit-tutorials/blob/3.7.0/advanced-features/openvidu-recording-basic-node-azure/public/app.js#L372-L379)
...
[index.html](https://github.com/OpenVidu/openvidu-livekit-tutorials/blob/3.7.0/advanced-features/openvidu-recording-basic-node-azure/public/index.html#L94-L99)
