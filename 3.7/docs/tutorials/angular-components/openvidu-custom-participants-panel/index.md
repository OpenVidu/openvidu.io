...
[Source code](https://github.com/OpenVidu/openvidu-tutorials/tree/3.7.0/openvidu-components-angular/openvidu-custom-participants-panel)
...
1. `localParticipant`: Local participant model.
1. `remoteParticipants`: Remote participants model.
1. `localParticipantSubs`: Subscription to the local participant updates.
1. `remoteParticipantsSubs`: Subscription to the remote participants updates.
1. `onTokenRequested` method that fires when the VideoconferenceComponent requests a token to connect to the OpenVidu Room.
1. `subscribeToParticipants` method that subscribes to updates for local and remote participants.
