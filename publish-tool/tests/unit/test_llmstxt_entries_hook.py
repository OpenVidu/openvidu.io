"""The llms.txt entries hook: both halves of an entry come from the page's frontmatter.
...
break. Pinned: both values come from the page, a listed page missing either fails the build, and a
plugin no longer exposing those attributes fails the build too rather than quietly publishing an
llms.txt full of nav labels and no descriptions.
...
from llmstxt_entries_hook import on_page_content
